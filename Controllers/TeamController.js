...
exports.createTeam = async (req, res, next) => {
    ...
    if (userId != null) {
        ...
        let newTeamId = await TeamHelper.createTeam(teamViewName, teamNickName, imagePath, userId);
...
exports.createTeamInvite = async (req, res, next) => {
    ...
    if (userId != null) {
        ...
        let teamInviteCheck = await TeamHelper.findInvite(request.teamId, userId, request.friendId);
        if (teamInviteCheck != null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_REQUESTED);
            return res.status(response.code).json(response);
        }
        if (teamInviteCheck == null) {
            let teamInviteId = await TeamHelper.createTeamInvite(request.teamId, userId, request.friendId)
            response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL);
            return res.status(response.code).json(response);
        }
    }
...
exports.teamInvitationResponse = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let checkTeamParticipatingInLadder = await LadderHelper.findTeamParicipatingInLadder(request.teamId)
        let teamInviteCheck = await TeamHelper.findRequestedInvite(request.teamId, userId, request.fromId);
        if (teamInviteCheck == null) {
            ...
            if (teamDetail != null) {
                ...
                let checkTeamParticipatingInTournament = await TournamentHelper.findTeamParticipating(request.teamId)
                if (checkTeamParticipatingInTournament.length > 0) {
                    await TeamHelper.acceptInvitation(request.teamId, userId, request.fromId, "cancelled");
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_JOIN_TEAM_PARTICIPATING_TOURNAMENT)
                    return res.status(response.code).json(response)
                } else {
                    if (checkTeamParticipatingInLadder.length > 0) {
                        for (let i = 0; i < checkTeamParticipatingInLadder.length; i++) {
                            ...
                            let ladderEndingDateAndTime = checkTeamParticipatingInLadder[i].endingDateAndTime
                    ...
                    if (ladderEndResultArr.length > 0) {
                        await TeamHelper.acceptInvitation(request.teamId, userId, request.fromId, "cancelled");
                        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_JOIN_TEAM_PARTICIPATING_LADDER)
                        return res.status(response.code).json(response)
                    } else {
                        await TeamHelper.acceptInvitation(request.teamId, userId, request.fromId, request.status);
...
exports.kickOutMember = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (this.checkAdmin(leader, request.teamName) == null) {
            response = await ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.PERMISSION_DENIED);
            return res.status(response.code).json(response);
        }
        let result = await TeamHelper.kickMember(request.userName, request.teamName);
        response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL, result);
        return res.status(response.code).json(response);
    }
}
...
exports.kickOutFromTeam = async (req, res) => {
    ...
    if (userId != null) {
        let checkTeamCreator
            = await TeamHelper.isTeamLeader(request, userId)
        let checkLeaderKickOut = await TeamHelper.isTeamLeader(request, request.teamMemberId)
        if (checkLeaderKickOut != null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TEAM_LEADER_NOT_KICK_OUT)
            return res.status(response.code).json(response);
        }
        if (checkTeamCreator
            == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TEAM_LEADER_CAN_KICK_OUT)
            return res.status(response.code).json(response);
        }
        if (checkTeamCreator != null) {
            ...
            let checkTeamParticipatingInTournament = await TournamentHelper.findTeamParticipating(request.teamId)
            if (checkTeamParticipatingInTournament.length > 0) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_KICK_TEAM_PARTICIPATING_TOURNAMENT)
                return res.status(response.code).json(response)
            } else {
                let checkTeamParticipatingInLadder = await LadderHelper.findTeamParicipatingInLadder(request.teamId)
                if (checkTeamParticipatingInLadder.length == 0) {
                    await TeamHelper.leaveTeamMember(request.teamId, request.teamMemberId)
                    response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                    ...
                    if (pendingResultArr.length > 0) {
                        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_KICK_TEAM_PARTICIPATING_LADDER)
                        return res.status(response.code).json(response)
                    } else {
                        await TeamHelper.leaveTeamMember(request.teamId, request.teamMemberId)
                        response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
...
exports.leaveTeam = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (checkTeamLeader != null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LEADER_CAN_NOT_LEAVE_TEAM)
            return res.status(response.code).json(response);
        }
        if (checkTeamLeader == null) {
...
exports.updateTeamTileImage = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (checkTeamCreator
            == null) {
            fs.unlinkSync(teamTitleImagePath);
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LEADER_CHANGE)
            return res.status(response.code).json(response);
        }
        if (checkTeamCreator
            != null) {
...
exports.updateTeamViewName = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (checkTeamCreator
            != null) {
            await TeamHelper.updateTeamViewName(request.teamId, request.teamViewName.toLowerCase().trim())
            response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
            response.teamData = await this.teamDetailData(request.teamId)
            return res.status(response.code).json(response);
        }
...
exports.deleteTeam = async (req, res) => {
    ...
    if (userId != null) {
        let checkTeamLeader
            = await TeamHelper.isTeamLeader(request, userId)
        if (checkTeamLeader == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LEADER_DELETE)
            return res.status(response.code).json(response);
        }
        if (checkTeamLeader != null) {
...
exports.createFranchiseTeam = async (req, res, next) => {
    ...
    if (userId != null) {
        ...
        let franchiseTeamResult = await TeamHelper.findFranchiseTeamViewNameWithoutDeleted(teamViewName, request.franchiseId)
        ...
        if (franchiseTeamResult == null) {
            ...
            let franchiseTeamId = await TeamHelper.createFranchiseTeam(teamViewName,
                teamNickName,
                imagePath,
                request.franchiseId);
            await FranchiseHelper.addTeamToFranchise(request.franchiseId, franchiseTeamId)
...
exports.createFranchiseTeamInvite = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (teamInviteCheck == null) {
            let checkFriendIsFranchiseOwner = await FranchiseHelper.getUserFranchiseDeatil(request.friendId)
            ...
            if (checkFriendIsFranchiseOwner != null) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.FRANCHISE_OWNER_NOT_REQUESTED + " of Franchise '" + franchiseName + "'");
                return res.status(response.code).json(response);
            }
            if (checkFriendIsFranchiseOwner == null) {
                let checkTeamMemberExistArr = await this.checkFranchiseTeamMemberDetail(request.friendId)//e,n
    ...
    if (userId != null) {
        ...
        if (teamInviteCheck != null) {
            ...
            if (teamMemberArr.includes(userId.toString())) {
                ...
                let teamMemberStringArr = await this.checkFranchiseTeamMemberDetail(userId)//e,n
...
exports.kickOutFromFranchiseTeam = async (req, res) => {
    ...
    if (userId != null) {
        let checkLeaderKickOut = await TeamHelper.isTeamLeader(request, request.teamMemberId)
        if (checkLeaderKickOut != null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS,
        ...
        if (franchiseTeamId == null) {
            ...
            let franchiseDetail = await FranchiseHelper.findFranchiseByIdWithoutDelete(franchiseTeamId)
            if (userId.toString() != franchiseDetail.createdBy.toString()) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ONLY_FRANCHISE_OWNER_CAN_KICK_OUT)
                return res.status(response.code).json(response);
            }
            let checkTeamParticipatingInTournament = await TournamentHelper.findTeamParticipating(request.teamId)
            if (checkTeamParticipatingInTournament.length > 0) {
                ...
                let checkTeamParticipatingInLeague = await LeagueHelper.findTeamParticipating(request.teamId)
                if (checkTeamParticipatingInLeague.length > 0) {
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_KICK_TEAM_PARTICIPATING_LEAGUE)
                    return res.status(response.code).json(response)
                } else {
                    if (userId.toString() == franchiseDetail.createdBy.toString()) {
                        await TeamHelper.leaveTeamMember(request.teamId, request.teamMemberId)
...
exports.deleteFranchiseTeam = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (franchiseId == null) {
            ...
            if (userId.toString() != franchiseDetail.createdBy.toString()) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ONLY_FRANCHISE_OWNER_CAN_DELETE)
                return res.status(response.code).json(response);
            } else {
                if (userId.toString() == franchiseDetail.createdBy.toString()) {
                    let checkFranchiseExist = await TeamHelper.findTeamIsPlayLeague(request.teamId)
                    if (checkFranchiseExist.length > 0) {
                        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TEAM_PARTICIPATE_IN_MATCH)
                        return res.status(response.code).json(response);
                    } else {
                        let checkPendingTeamInviteExist = await TeamHelper.findPendingTeamInvite(request.teamId)
                        if (checkPendingTeamInviteExist.length > 0) {
                            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TEAM_HAS_PENDING_INVITES)
                            return res.status(response.code).json(response);
                        } else {
                            await TeamHelper.deleteTeam(request.teamId)
                            response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
...
exports.getTeamRoosterByTeamId = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (teamDetail != null) {
            if (teamDetail.teamMembers.length > 0) {
                for (let i = 0; i < teamDetail.teamMembers.length; i++) {
                    let userData = await UserHelper.foundUserById(teamDetail.teamMembers[i].userId)
                    let userObj = {
                        _id: userData._id,
            ...
            if (teamMembers.length > 0) {
                for (let i = 0; i < teamMembers.length; i++) {
                    if (teamMembers[i].userId.toString() == memberId.toString()) {
                        let fullName
                        let profileImage
                        let userData = await UserHelper.foundUserById(teamMembers[i].userId)
                        if (userData != null) {
                            fullName = userData.userDetail.userName
                            profileImage = userData.profileImage
                            matchData = await MatchController.matchDataByUserId(teamMembers[i].userId)
                        } else {
                            fullName = ""
                            profileImage = ""
                        }
...
exports.getFranchiseTeamList = async (req, res) => {
    ...
    if (userId != null) {
        let teamArr = []
        let franchiseTeamList = await TeamHelper.findFranchiseTeamWithoutDeleted(franchiseId)
...
exports.myTeamsData = async (result) => {
    ...
    if (result.length > 0) {
        for (let i = 0; i < result.length; i++) {
            teamId = result[i]._id
            let userParticipatingTournaments = await TournamentHelper.findMyTournament(teamId)
            if (userParticipatingTournaments.length > 0) {
                for (let j = 0; j < userParticipatingTournaments.length; j++) {
                    if (userParticipatingTournaments[j].winningTeam != null) {
                        if (userParticipatingTournaments[j].winningTeam.toString() != teamId.toString()) {
                            loss++
                        }
                        if (userParticipatingTournaments[j].winningTeam.toString() == teamId.toString()) {
                            win++
...
exports.teamDetailData = async (teamId) => {
    ...
    let teamParticipatingTournaments = await TournamentHelper.findMyTournament(teamId)
    ...
    if (teamData.teamLeader == null) {
        ...
        let matchData = await MatchController.matchDataByUserId(teamData.teamLeader._id)
        leader = {
...
exports.checkFranchiseTeamMemberDetail = async (userId) => {
    ...
    for (let i = 0; i < franchiseDetail.length; i++) {
        ...
        for (let j = 0; j < franchiseTeamsArr.length; j++) {
            ...
            let teamData = await TeamHelper.findTeamDetailByTeamIdWithoutDelete(teamId)
            if (teamData != null) {
                if (teamData.teamMembers.length > 0) {
