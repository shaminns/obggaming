...
exports.createFantasyLeague = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (!request.flName, !request.teamSize, !request.totalTeams, !request.draftDateAndTime, !request.leagueId) {
            response = ResponseHelper.setResponse(
                ResponseCode.NOT_SUCCESS, Message.MISSING_PARAMETER
            )
            return res.status(response.code).json(response);
        }
        let leagueDetail = await LeagueHelper.findLeagueByIdWithDeleteWithPopulate(request.leagueId)
        ...
        if (!dateCheckResult) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.CAN_NOT_CREATE_FL_LEAGUE_END)
            return res.status(response.code).json(response);
        } else {
            ...
            let teamSizeCalculation = (parseInt(leagueTotalTeams) * parseInt(leagueTeamSize)) / parseInt(request.totalTeams)
            ...
            if (teamCountResult === false) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TOTAL_TEAMS_ERROR);
                return res.status(response.code).json(response);
            }
            if (request.totalTeams < 4) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.MINIMUM_TEAM_REQUIRED);
                return res.status(response.code).json(response);
            }
            if (request.totalTeams > leagueTotalTeams) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, "Total team must be equal or less than " + leagueTotalTeams);
                return res.status(response.code).json(response);
            }
            if (request.teamSize > requiredFlTeamSize) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, "Maximum team size must be " + requiredFlTeamSize);
                return res.status(response.code).json(response);
            }
            let fantasyLeagueNameCheck = await FantasyLeagueHelper.findFantasyLeagueByNameWithoutDelete(request.flName.toLowerCase().trim())
            if (userRole == "Admin") {
                flType = "public"
            } else {
                flType = "private"
            ...
            if (fantasyLeagueNameCheck != null) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NAME_ALREADY_EXIST);
                return res.status(response.code).json(response);
            } else {
                ...
                let randomNumber = await this.getRandomNmmber()
                ...
                if (userRole == "User") {
                    let userFantasyTeamId = await this.createFantasyTeamForUser(userId, fantasyLeagueId, request.teamSize, request.totalTeams)
                    await FantasyLeagueHelper.addFantasyTeam(fantasyLeagueId, userFantasyTeamId)
                    let fantasyLeagueDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(fantasyLeagueId)
                    flData = {
                ...
                if (userRole == "Admin") {
                    let fantasyLeagueDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(fantasyLeagueId)
                    let fantasyLeagueYear = moment(fantasyLeagueDetail.createdAt).format('yyyy');
                    flData = {
                        _id: fantasyLeagueDetail._id,
                        flName: fantasyLeagueDetail.flName,
                        flTitleImage: fantasyLeagueDetail.flTitleImage,
...
exports.showAllFantasyLeague = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let joinFlArrByGame = []
        if (!req.query.game) {
            allPublicFantasyLeague = await FantasyLeagueHelper.findAllPublicFantasyLeagues()
            allPrivateFantasyLeague = await FantasyLeagueHelper.findAllPrivateFantasyLeagues(userId)
            let userAllFantasyLeague = await FantasyTeamHelper.findAllFantasyTeamByUserId(userId)
            if (userAllFantasyLeague.length > 0) {
                for (let i = 0; i < userAllFantasyLeague.length; i++) {
                    ...
                    let leagueDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(leagueId)
                    let createdBy = leagueDetail.createdBy
                    if (createdBy.toString() != userId.toString()) {
                        let joinedFl = await FantasyLeagueHelper.findFantasyLeagueByFlTeam(flTeamId)
        ...
        if (req.query.game) {
            ...
            if (parseGameName.toString() == "all") {
                ...
                allPublicFantasyLeague = await FantasyLeagueHelper.findAllPublicFantasyLeaguesByGameName(gameName)
                allPrivateFantasyLeague = await FantasyLeagueHelper.findAllPrivateFantasyLeaguesByGameName(userId, gameName)
                let userAllFantasyLeague = await FantasyTeamHelper.findAllFantasyTeamByUserId(userId)
                if (userAllFantasyLeague.length > 0) {
                    for (let i = 0; i < userAllFantasyLeague.length; i++) {
                        let flTeamId = userAllFantasyLeague[i]._id
        ...
        response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
        response.flData = {publicFl: finalPublicFlArr, privateFl: finalPrivateFlArr, joinedFl: finalJoinedFlArr}
        return res.status(response.code).json(response);
    }
}
exports.getFantasyLeagueById = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let userFantasyLeagueDetail = await this.userFantasyLeagueDetailData(userId, fantasyLeagueId)
        response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
...
exports.joinFantasyLeague = async (req, res) => {
    ...
    if (userId != null) {
        let flDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(request.fantasyLeagueId)
        ...
        if (registeredTeams >= requiredTeams) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.REQUIRED_TEAMS_ALREADY_REGISTERED)
            return res.status(response.code).json(response);
        } else {
            let checkAlreadyJoined = await FantasyTeamHelper.checkAlreadyHaveTeam(userId, request.fantasyLeagueId)
            if (checkAlreadyJoined != null) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_JOINED)
                return res.status(response.code).json(response);
            } else {
                let userFantasyTeamId = await this.createFantasyTeamForUser(userId, request.fantasyLeagueId, teamSize, registeredTeams)
                await FantasyLeagueHelper.addFantasyTeam(request.fantasyLeagueId, userFantasyTeamId)
                let userFantasyLeagueDetail = await this.userFantasyLeagueDetailData(userId, request.fantasyLeagueId)
                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                response.fantasyLeagueData = userFantasyLeagueDetail
                return res.status(response.code).json(response);
            }
...
exports.sendFlJoinInvitation = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let flInviteCheck = await FantasyLeagueHelper.findInvite(request.fantasyLeagueId, userId, request.friendId);
        if (flInviteCheck != null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_REQUESTED);
            return res.status(response.code).json(response);
        }
        if (flInviteCheck == null) {
            let flJoinCheck = await this.fantasyLeagueJoinCheckForUser(request.friendId, request.fantasyLeagueId)
            if (flJoinCheck == true) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_MEMBER);
                return res.status(response.code).json(response);
            } else {
                await FantasyLeagueHelper.createFlInvite(request.fantasyLeagueId, userId, request.friendId)
                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                return res.status(response.code).json(response);
            }
        }
...
exports.responseFlJoinInvitaion = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let requiredTeams = flDetail.totalTeams
        let teamSize = flDetail.teamSize
        let flInviteCheck = await FantasyLeagueHelper.findRequestedInvite(request.fantasyLeagueId, userId, request.fromId);
        if (flInviteCheck == null) {
            response = await ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_REQUESTED);
            return res.status(response.code).json(response);
        } else {
            if (registeredTeams >= requiredTeams) {
                let status = "dropped"
                await FantasyLeagueHelper.updateInvitationStatus(request.fantasyLeagueId, userId, request.fromId, status)
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.REQUIRED_TEAMS_ALREADY_REGISTERED)
                return res.status(response.code).json(response);
            } else {
...
exports.showAllLeaguePlayer = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let leagueDetail = await LeagueHelper.findLeagueByIdWithoutDeleteWithPopulate(leagueId)
        if (leagueDetail != null) {
            ...
            if (leagueTeams.length > 0) {
                for (let i = 0; i < leagueTeams.length; i++) {
                    ...
                    let teamDetail = await TeamHelper.findTeamDeatilByTeamId(leagueTeams[i])
                    if (teamDetail != null) {
        ...
        finalPlayerArr = playerIdArr.filter(val => !selectedPlayersArr.includes(val))
...
exports.addMemberToFantasyLeague = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let flTeamId = request.flTeamId
        let alreadyTeamMemberCheck = await FantasyTeamHelper.checkAlreadyTeamMember(flTeamId, playerId)
        if (alreadyTeamMemberCheck != null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_SELECTED)
            return res.status(response.code).json(response);
        } else {
            let teamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(flTeamId)
...
exports.updateMemberToFantasyLeague = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (alreadyTeamMemberCheck != null) {
            ...
            await FantasyTeamHelper.updateMemberToFlTeam(flTeamId, playerId, oldPlayerId)
            let userData = await UserHelper.foundUserById(playerId)
            let newUserObj = {
                _id: userData._id,
                userName: userData.userDetail.userName,
...
exports.showAllPublicFantasyLeague = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (!req.query.query) {
            allPublicFantasyLeague = await FantasyLeagueHelper.allFantasyLeaguesWithPagination(pageNo)
        }
        if (req.query.query) {
            let searchStr = req.query.query.toLowerCase().trim()
            allPublicFantasyLeague = await FantasyLeagueHelper.searchFantasyLeagueByNameWithPagination(searchStr, pageNo)
        }
        if (allPublicFantasyLeague.data.length > 0) {
            for (let i = 0; i < allPublicFantasyLeague.data.length; i++) {
                let fantasyLeagueYear = moment(allPublicFantasyLeague.data[i].createdAt).format('yyyy');
                let flObj = {
                    ...
                    winner: allPublicFantasyLeague.data[i].winner,
...
exports.editFantasyLeague = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (fantasyLeagueRegisterTeam > 0) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_UPDATE_NOW)
            return res.status(response.code).json(response);
        } else {
            let flId = await FantasyLeagueHelper.updateFantasyLeagueDetail(request)
            let flDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(flId)
...
exports.deleteFantasyLeague = async (req, res) => {
    ...
    for (let i = 0; i < flIdArr.length; i++) {
        leagueId = flIdArr[i]
        let fantasyLeagueDetailById = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(leagueId)
    ...
    if (pendingId.length > 0) {
        response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.SOME_RECORDS_NOT_DELETE)
...
exports.changeFlTeamName = async (req, res) => {
    ...
    if (userId != null) {
        let flTeamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(flTeamId)
        if (flTeamDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TEAM_NOT_FOUND)
            return res.status(response.code).json(response);
        } else {
            let flId = flTeamDetail.teamType
            if (userId.toString() != flTeamDetail.teamOwner.toString()) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_AUTHORIZE_ACTION)
                return res.status(response.code).json(response);
            } else {
                await FantasyTeamHelper.updateFlTeamName(flTeamId, flTeamName)
                let userFantasyLeagueDetail = await this.userFantasyLeagueDetailData(userId, flId)
                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                response.fantasyLeagueData = userFantasyLeagueDetail
                return res.status(response.code).json(response);
            }
...
exports.getFlSchedule = async (req, res) => {
    ...
    if (userId != null) {
        let scheduleType
        let fantasyLeagueId = request.fantasyLeagueId
        let fantasyLeagueDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithDelete(fantasyLeagueId)
        if (fantasyLeagueDetail != null) {
            ...
            console.log("teams : ", fantasyLeagueRequiredTotalTeams, totalRegisteredTeams)
            if (totalRegisteredTeams < fantasyLeagueRequiredTotalTeams) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LEAGUE_PENDING_REGISTRATION)
                ...
                if (leagueRegisteredTeams < leagueTotalRequiredTeams) {
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LEAGUE_SLOTS_EMPTY)
                    return res.status(response.code).json(response);
                }
                ...
                if (dateCheckResult == false) {
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.GP_LEAGUE_NOT_STARTED)
                    return res.status(response.code).json(response);
                } else {
                    let findPendingWinnerRoundByLeagueIdAndRound = await LeagueResultHelper.findPendingWinnerRoundByLeagueIdAndRound(leagueId, 1)
                    if (findPendingWinnerRoundByLeagueIdAndRound.length > 0) {
                        console.log("can not start or play FL now")
                        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.CAN_NOT_PLAY_FL)
                        return res.status(response.code).json(response);
                    } else {
                        let alreadyFlScheduleCheck = await FantasyLeagueHelper.checkAlreadyFlScheduleByFlId(fantasyLeagueId)
                        console.log("fl schedule check")
                        if (alreadyFlScheduleCheck.length == 0) {
                            /////
                            console.log("fl schedule not exist - new generated")
                            scheduleType = "round"
                            let shuffledParticipatingTeamArr = this.shuffleFLArr(flTeams);
                            await this.generateFantasyLeagueSchedule(roundNumber, fantasyLeagueRequiredTotalTeams, shuffledParticipatingTeamArr, fantasyLeagueId, leagueId, scheduleType)
                            let leagueMatchScheduleDetail = await this.fantasyLeagueScheduleData(request.fantasyLeagueId)
                            response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                        ...
                        if (alreadyFlScheduleCheck.length > 0) {
                            ...
                            let fantasyLeagueDetailForMaxRoundNumber = await FantasyLeagueHelper.getLeagueDetailForMaxRoundNumber(fantasyLeagueId)
                            let maxRoundNumber = fantasyLeagueDetailForMaxRoundNumber[0].roundNumber
                            console.log("fanatsy league Schedule Maxx Round : ", maxRoundNumber)
                            let checkPendingFantasyLeagueScheduleWinnerByRound = await FantasyLeagueHelper.checkPendingFantasyLeagueScheduleWinner(fantasyLeagueId, maxRoundNumber)
                            console.log("Length Check Pending Fantasy League Schedule Winner By Round : ", checkPendingFantasyLeagueScheduleWinnerByRound.length)
                            if (checkPendingFantasyLeagueScheduleWinnerByRound.length > 0) {
                                console.log("pending fl schedule winner length > 0")
                                let checkAlreadyLeagueSchedulePendingResult = await LeagueScheduleHelper.checkLeagueRoundWinner(leagueId, maxRoundNumber)
                                console.log("length check already league schedule pending result : ", checkAlreadyLeagueSchedulePendingResult.length)
                                if (checkAlreadyLeagueSchedulePendingResult.length > 0) {
                                    console.log("league pending result length > 0")
                                ...
                                if (checkAlreadyLeagueSchedulePendingResult.length == 0) {
                                    console.log("league pending result length = 0")
                                    // console.log("fl winner declare and next schedule")
                                    // console.log("/////////////////Max : ", maxRoundNumber)
                                    // ///////////////////////////////////////////////////////////////////////////////////
                                    // console.log("fantasyLeagueId : ", fantasyLeagueId)
                                    // console.log("leagueId : ", leagueId)
                                    let flScheduleForPendingResult = await FantasyLeagueHelper.findSchedulePendingResult(fantasyLeagueId, maxRoundNumber)
                                    if (flScheduleForPendingResult.length > 0) {
                                        console.log("pending result for fl schedule length > 0")
                                        ...
                                        let scheduleTeamArr = []
                                        let flScheduleDetailForRound = await FantasyLeagueHelper.findScheduleOfRound(fantasyLeagueId, maxRoundNumber)
                                        // if (flScheduleDetailForRound.length > 0) {
                                        console.log("FL round result going to announce")
                                        console.log("fl schedule (round) array for getting teams")
                                        let maxRoundNumberFromFlScheduleWinnerDetail = await FantasyLeagueHelper.maxRoundNumberFromFlScheduleWinnerNull(fantasyLeagueId)
                                        let maxRoundNumberFromFlScheduleWinner = maxRoundNumberFromFlScheduleWinnerDetail[0].roundNumber
                                        console.log("max round number for fl schedule winner null : ", maxRoundNumberFromFlScheduleWinner)
                                        for (let i = 0; i < flScheduleDetailForRound.length; i++) {
                                            ...
                                            let teamOneDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(teamOneId)
                                            ...
                                            if (teamOneMembers.length > 0) {
                                                for (let j = 0; j < teamOneMembers.length; j++) {
                                                    ...
                                                    console.log("T1 player Id : ", j + 1, " ", playerId, "league id : ", leagueId, "max round no : ", maxRoundNumberFromFlScheduleWinner)
                                                    let playerLeaguePointsDetail = await FantasyLeagueHelper.getPlayerLeaguePointByRoundNumber(playerId, leagueId, maxRoundNumberFromFlScheduleWinner)
                                                    console.log("player 1 detail in point : ", playerLeaguePointsDetail)
                                                    if (playerLeaguePointsDetail == null) {
                                                        console.log("givig 0")
                                                        teamOneFinalPoints = teamOneFinalPoints + parseInt(0)
                                                    } else {
                                                        teamOneFinalPoints = teamOneFinalPoints + parseInt(playerLeaguePointsDetail.points)
                                                    }
                                                }
                                            }
                                            let teamTwoDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(teamTwoId)
                                            ...
                                            if (teamTwoMembers.length > 0) {
                                                for (let k = 0; k < teamTwoMembers.length; k++) {
                                                    ...
                                                    console.log("T2 player Id : ", k + 1, " ", playerId, "league id : ", leagueId, "max round no : ", maxRoundNumberFromFlScheduleWinner)
                                                    let playerLeaguePointsDetail = await FantasyLeagueHelper.getPlayerLeaguePointByRoundNumber(playerId, leagueId, maxRoundNumberFromFlScheduleWinner)
                                                    console.log("player 2 detail in point : ", playerLeaguePointsDetail)
                                                    if (playerLeaguePointsDetail == null) {
                                                        console.log("givig 0")
                                                        teamTwoFinalPoints = teamTwoFinalPoints + parseInt(0)
                                                    } else {
                                                        teamTwoFinalPoints = teamTwoFinalPoints + parseInt(playerLeaguePointsDetail.points)
                                                    }
                                                }
                                            }
                                            // /check already result save
                                            // let maxRoundNumberFromFlScheduleWinnerDetail1 = await FantasyLeagueHelper.maxRoundNumberFromFlScheduleWinner(fantasyLeagueId)
                                            // let maxRoundNumberFromFlScheduleWinner1 = maxRoundNumberFromFlScheduleWinnerDetail1[0].roundNumber
                                            let teamOnePointRecordId
                                            let teamTwoPointRecordId
                                            let teamOnePointsResultExist = await FantasyLeagueHelper.findAlreadyTeamPointsExistForScheduleRound(fantasyLeagueId, maxRoundNumber, teamOneId)
                                            if (teamOnePointsResultExist.length > 0) {
                                                console.log("Team One Points already exist")
                                            } else {
                                                console.log("Team One Points not exist")
                                                teamOnePointRecordId = await FantasyLeagueHelper.addFantasyTeamPoint(maxRoundNumber, teamOneFinalPoints,
                                                    teamOneId, leagueId, fantasyLeagueId)
                                            }
                                            ///
                                            let teamTwoPointsResultExist = await FantasyLeagueHelper.findAlreadyTeamPointsExistForScheduleRound(fantasyLeagueId, maxRoundNumber, teamTwoId)
                                            if (teamTwoPointsResultExist.length > 0) {
                                                console.log("Team Two Points already exist")
                                            } else {
                                                console.log("Team Two Points not exist")
                                                teamTwoPointRecordId = await FantasyLeagueHelper.addFantasyTeamPoint(maxRoundNumber, teamTwoFinalPoints,
                                                    teamTwoId, leagueId, fantasyLeagueId)
                                            }
                                            if (teamOneFinalPoints == teamTwoFinalPoints) {
                                                console.log("same points")
                                                console.log("teamOne : ", teamOneId)
                                                console.log("teamTwo : ", teamTwoId)
                                                let leaderboardPointsDetailArr = await this.leaderboardDataDetail(fantasyLeagueId)
                                                console.log("leaderboard detail arr : ", leaderboardPointsDetailArr)
                                                let teamOneLeaderBoardDetail = leaderboardPointsDetailArr.filter(x => x._id.toString() == teamOneId.toString())
                                                console.log("team one leader board detail : ", teamOneLeaderBoardDetail)
                                                let teamTwoLeaderBoardDetail = leaderboardPointsDetailArr.filter(x => x._id.toString() == teamTwoId.toString())
                                                console.log("team two leader board detail : ", teamTwoLeaderBoardDetail)
                                                let teamOneLeaderboardPoints = teamOneLeaderBoardDetail[0].points
                                                let teamTwoLeaderboardPoints = teamTwoLeaderBoardDetail[0].points
                                                if (teamOneLeaderboardPoints > teamTwoLeaderboardPoints) {
                                                    console.log("win team one with leaderboard")
                                                    await FantasyLeagueHelper.updateFantasyLeagueScheduleResult(maxRoundNumber, fantasyLeagueId, teamOneId)
                                                    await FantasyLeagueHelper.updateTeamPointWinner(fantasyLeagueId, maxRoundNumber, teamOneId)
                                                    await FantasyLeagueHelper.updateTeamPointLooser(fantasyLeagueId, maxRoundNumber, teamTwoId)
                                                } else {
                                                    console.log("win team two with leaderboard")
                                                    await FantasyLeagueHelper.updateFantasyLeagueScheduleResult(maxRoundNumber, fantasyLeagueId, teamTwoId)
                                                    await FantasyLeagueHelper.updateTeamPointWinner(fantasyLeagueId, maxRoundNumber, teamTwoId)
                                                    await FantasyLeagueHelper.updateTeamPointLooser(fantasyLeagueId, maxRoundNumber, teamOneId)
                                                }
                                        ...
                                        /////genrate schedule
                                        //********************************************************************************
                                        console.log("fl schedule result announced going to generate next schedule")
                                        // let pendingFantasyWinnerList = await FantasyLeagueHelper.findPendingWinnerList(fantasyLeagueId, maxRoundNumber)
                                        // if (pendingFantasyWinnerList.length == 0) {
                                        if (fantasyLeagueRequiredTotalTeams == 4 && maxRoundNumber == 1) {
                                            console.log("total teams = 4 and round = 1")
                                            //
                                            let firstRoundFourTeamsArr = await FantasyLeagueHelper.getPreviousRoundFourTeamList(fantasyLeagueId, leagueId, maxRoundNumber)
                                            console.log("first Round 4 teams : ", firstRoundFourTeamsArr)
                                            previousRoundWinnerTeamsArr = firstRoundFourTeamsArr
                                        } else {
                                            console.log("total teams != 4 and round != 1")
                                            previousRoundWinnerTeamsArr = await FantasyLeagueHelper.getPreviousRoundWinnerList(fantasyLeagueId, leagueId, maxRoundNumber)
                                        }
                                        if (previousRoundWinnerTeamsArr.length == 32 || previousRoundWinnerTeamsArr.length == 16 ||
                                            previousRoundWinnerTeamsArr.length == 8 || previousRoundWinnerTeamsArr.length == 4 ||
                                            previousRoundWinnerTeamsArr.length == 2 || previousRoundWinnerTeamsArr.length == 1) {
                                            for (let i = 0; i < previousRoundWinnerTeamsArr.length; i++) {
                                                teamArr.push(previousRoundWinnerTeamsArr[i].flTeamId)
                                            }
                                            console.log("teamArr from previous round record : ", teamArr)
                                            console.log("teamArr length : ", previousRoundWinnerTeamsArr.length)
                                            if (previousRoundWinnerTeamsArr.length > 4) {
                                                ...
                                                await this.generateFantasyLeagueSchedule(roundNumber, totalTeams, teamArr, fantasyLeagueId, leagueId, scheduleType)
                                                let leagueMatchScheduleDetail = await this.fantasyLeagueScheduleData(request.fantasyLeagueId)
                                                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                                                response.leagueScheduleData = leagueMatchScheduleDetail
                                                response.message = Message.CHECK_NEW_SCHEDULE
                                            ...
                                            if (previousRoundWinnerTeamsArr.length == 4) {
                                                ...
                                                //
                                                let leagueMatchScheduleDetail = await this.scheduleForFourTeams(teamArr, maxRoundNumber, fantasyLeagueId, leagueId)
                                                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                                                response.leagueScheduleData = leagueMatchScheduleDetail
                                                response.message = Message.CHECK_NEW_SCHEDULE
                                                return res.status(response.code).json(response);
                                            ...
                                            if (previousRoundWinnerTeamsArr.length == 2) {
                                                ...
                                                let leagueMatchScheduleDetail = await this.scheduleForTwoTeams(maxRoundNumber, fantasyLeagueId, leagueId)
                                                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                                                response.leagueScheduleData = leagueMatchScheduleDetail
                                                response.message = Message.CHECK_NEW_SCHEDULE
                                                return res.status(response.code).json(response);
                                            ...
                                            if (previousRoundWinnerTeamsArr.length == 1) {
                                                ...
                                                let checkLeagueFinalExist = await FantasyLeagueHelper.checkLeagueFinalExist(fantasyLeagueId)
                                                console.log("check fl final exist : ", checkLeagueFinalExist)
                                                if (checkLeagueFinalExist == null) {
                                                    console.log("check fl final exist is null ")
                                                    console.log("1 schedule")
                                                    let leagueMatchScheduleDetail = await this.scheduleForOneTeam(maxRoundNumber, previousRoundWinnerTeamsArr, fantasyLeagueId, leagueId)
                                                    response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                                                    response.leagueScheduleData = leagueMatchScheduleDetail
                                                    response.message = Message.CHECK_NEW_SCHEDULE
                                                    return res.status(response.code).json(response);
                                                ...
                                                if (checkLeagueFinalExist != null) {
                                                    ...
                                                    let fantasyLeagueDetails = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(fantasyLeagueId)
                                                    if (fantasyLeagueDetails.winningTeam == null) {
                                                        console.log("fl schedule winner not exist and update winner")
                                                        await FantasyLeagueHelper.updateWinnerTeam(fantasyLeagueId, winnigTeamId)
                                                    }
                                                    let winningTeamName = await this.getFantasyTeamName(winnigTeamId)
                                                    console.log("no schedule required")
                                                    let leagueMatchScheduleDetail = await this.scheduleForOneTeam(maxRoundNumber, previousRoundWinnerTeamsArr, fantasyLeagueId, leagueId)
                                                    response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                                                    response.leagueScheduleData = leagueMatchScheduleDetail
                                                    response.message = "WooHoo Congratulation! " + winningTeamName + " has won this league."
                                                    return res.status(response.code).json(response);
                                                }
                                            }
                                        } else {
...
exports.getFlStat = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let finalStatsArr = []
        let maxRoundNumberFromFlScheduleDetail = await FantasyLeagueHelper.getLeagueDetailForMaxRoundNumber(fantasyLeagueId)
        if (maxRoundNumberFromFlScheduleDetail.length > 0) {
            maxRoundNumberFromFlSchedule = maxRoundNumberFromFlScheduleDetail[0].roundNumber
        }
        let fantasyLeagueDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithDelete(fantasyLeagueId)
        if (fantasyLeagueDetail != null) {
            let fantasyLeagueRequiredTotalTeams = fantasyLeagueDetail.totalTeams
            ...
            let leaguePlayerPointsDetailArr = await LeagueHelper.getPlayerPointsDetailByLeague(leagueId, roundNumber)
            if (leaguePlayerPointsDetailArr.length > 0) {
                let fantasyLeaguePlayerList = await this.getFantasyLeaguePlayersList(fantasyLeagueId)
                if (fantasyLeaguePlayerList.length > 0) {
                    for (let i = 0; i < fantasyLeaguePlayerList.length; i++) {
                        ...
                        for (let j = 0; j < leaguePlayerPointsDetailArr.length; j++) {
                            if (playerId.toString() == leaguePlayerPointsDetailArr[j].playerId.toString()) {
                                finalPlayerPoints = parseInt(finalPlayerPoints) + parseInt(leaguePlayerPointsDetailArr[j].points)
                            }
                        }
                        playerWithPointsArr.push({playerId: playerId, finalPoints: finalPlayerPoints})
                    }
                }
            }
        }
        if (playerWithPointsArr.length > 0) {
            sortedPlayerPointsArr = playerWithPointsArr.sort((a, b) => b.finalPoints - a.finalPoints)
            ...
            for (let m = 0; m < sortedPlayerPointsArr.length; m++) {
                ...
                let playerFlTeamDetail = await FantasyTeamHelper.findPlayerAllFantayTeam(playerIdForPick)
                let pickCount = playerFlTeamDetail.length
                let pickPercentage = ((parseInt(pickCount) / parseInt(totalFlTeamCount)) * 100).toFixed(0)
...
exports.flTeamsDetail = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let maxRoundNumberFromFlSchedule = 1
        let flTeamScheduleDetail = await FantasyLeagueHelper.flTeamsDetailFromScheduel(recordId)
        ...
        if (flTeamScheduleDetail != null) {
            ...
            let finalTeamOneArr = await this.teamDetailWithPoints(teamOneId, leagueId, fantasyLeagueId, maxRoundNumberFromFlSchedule)
            let finalTeamTwoArr = await this.teamDetailWithPoints(teamTwoId, leagueId, fantasyLeagueId, maxRoundNumberFromFlSchedule)
            response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
...
exports.getTradeMovePlayerData = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let fantasyLeagueId = request.fantasyLeagueId
        let userFlTeamDetail = await FantasyTeamHelper.findMyFantasyTeamDetailByFlId(userId, fantasyLeagueId)
        if (userFlTeamDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.USER_TEAM_NOT_FOUND)
            return res.status(response.code).json(response);
        } else {
            giveFlTeamId = userFlTeamDetail._id
        }
        let flTeamDetailForAlreadyCheck = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(takePlayerFlTeamId)
        ...
        let alreadyProposalDetail = await TradeMoveHelper.findAlreadySendProposal(takePlayerFlTeamId, giveFlTeamId, fantasyLeagueId, toId, fromId, takePlayerId)
        if (alreadyProposalDetail != null) {
            let givePlayerId = alreadyProposalDetail.givePlayerId
            let takePlayerDataArr = await this.playerCardData(takePlayerId, fantasyLeagueId)
            let givePlayerDataArr = await this.playerCardData(givePlayerId, fantasyLeagueId)
            let playerDataArr = []
            playerDataArr.push(takePlayerDataArr[0])
            playerDataArr.push(givePlayerDataArr[0])
            response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
            response.tradeMoveData = {playerData: playerDataArr, aprrovedStatus: alreadyProposalDetail.approvedStatus}
            return res.status(response.code).json(response);
        } else {
            ...
            let playerDataArr = await this.playerCardData(takePlayerId, fantasyLeagueId)
            playerDataArr.push(emptyObjForGivePlayer)
            let flTeamDetail = await FantasyTeamHelper.findMyFantasyTeamDetailByFlId(userId, fantasyLeagueId)
            let maxRoundNumberFromFlSchedule = 1
            let flDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(fantasyLeagueId)
            if (flDetail != null) {
                leagueId = flDetail.league
                let maxRoundNumberFromFlScheduleDetail = await FantasyLeagueHelper.getLeagueDetailForMaxRoundNumber(fantasyLeagueId)
                if (maxRoundNumberFromFlScheduleDetail.length > 0) {
                ...
                let finalTeamOneArr = await this.teamDetailWithPoints(teamId, leagueId, fantasyLeagueId, maxRoundNumberFromFlSchedule)
                ...
                if (teamDataWithPoints.length > 0) {
                    for (let k = 0; k < teamDataWithPoints.length; k++) {
                        ...
                        let givePlayerRequest = await TradeMoveHelper.findGivePlayerForThisPlayer(fantasyLeagueId, playerId)
                        let takePlayerRequest = await TradeMoveHelper.findTakePlayerForThisPlayer(fantasyLeagueId, playerId)
                ...
                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                response.tradeMoveData = {playerData: playerDataArr, playerList: finalTeamArrWithDetail}
                return res.status(response.code).json(response);
            }
        }
    }
...
exports.sendTradeProposal = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let takePlayerId = request.takePlayerId
        let flTeamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(takePlayerFlTeamId)
        if (flTeamDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TEAM_NOT_FOUND)
            return res.status(response.code).json(response);
        } else {
        ...
        let userFlTeamDetail = await FantasyTeamHelper.findMyFantasyTeamDetailByFlId(userId, flId)
        let giveFlTeamId
        let giveFlTeamName
        if (userFlTeamDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.USER_TEAM_NOT_FOUND)
        ...
        let alreadyProposalDetail = await TradeMoveHelper.findAlreadyProposal(takePlayerFlTeamId, giveFlTeamId, flId, toId, fromId, givePlayerId, takePlayerId)
        if (alreadyProposalDetail != null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_REQUESTED)
            return res.status(response.code).json(response);
        } else {
            await TradeMoveHelper.addTradeMoveProposal(takePlayerFlTeamId, giveFlTeamId, flId, toId, fromId, givePlayerId, takePlayerId)
            ...
            let fantasyLeagueDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(flId)
            let flName
            if (fantasyLeagueDetail != null) {
                flName = fantasyLeagueDetail.flName
            } else {
            ...
            EmailHelper.sendEmail(email, subject, message, 'tradeProposal', replacements);
            ////
            response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
...
exports.userTradeMoveRequest = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let finalSendArr = []
        let receivedTradeMoveRequest = await TradeMoveHelper.findUserReceivedTradeMoveRequest(userId)
        let sendTradeMoveRequest = await TradeMoveHelper.findUserSendTradeMoveRequest(userId)
        if (receivedTradeMoveRequest.length > 0) {
            for (let i = 0; i < receivedTradeMoveRequest.length; i++) {
                let userToDetail = await UserHelper.foundUserById(receivedTradeMoveRequest[i].toId);
                let userToUserName = userToDetail.userDetail.userName;
                let userFromDetail = await UserHelper.foundUserById(receivedTradeMoveRequest[i].fromId);
                let userFromUserName = userFromDetail.userDetail.userName;
                let givePlayerDetail = await UserHelper.foundUserById(receivedTradeMoveRequest[i].givePlayerId)
                let givePlayerUserName = givePlayerDetail.userDetail.userName
                let takePlayerDetail = await UserHelper.foundUserById(receivedTradeMoveRequest[i].takePlayerId)
        ...
        if (sendTradeMoveRequest.length > 0) {
            for (let i = 0; i < sendTradeMoveRequest.length; i++) {
                let userToDetail = await UserHelper.foundUserById(sendTradeMoveRequest[i].toId);
                let userToUserName = userToDetail.userDetail.userName;
                let userFromDetail = await UserHelper.foundUserById(sendTradeMoveRequest[i].fromId);
                let userFromUserName = userFromDetail.userDetail.userName;
                let givePlayerDetail = await UserHelper.foundUserById(sendTradeMoveRequest[i].givePlayerId)
                let givePlayerUserName = givePlayerDetail.userDetail.userName
                let takePlayerDetail = await UserHelper.foundUserById(sendTradeMoveRequest[i].takePlayerId)
                let takePlayerUserName = takePlayerDetail.userDetail.userName
                finalSendArr.push({
        ...
        if (finalReceivedArr.length > 0) {
            sortedReceivedArray = finalReceivedArr.sort((a, b) => new moment(b.createdAt) - new moment(a.createdAt))
        }
        if (finalSendArr.length > 0) {
            sortedSendArray = finalSendArr.sort((a, b) => new moment(b.createdAt) - new moment(a.createdAt))
        }
        response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
...
exports.tradeMoveRequestDetail = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let tradeMoveRequestDetail = await TradeMoveHelper.findTradeMoveReqById(recordId)
        if (tradeMoveRequestDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.RECORD_NOT_FOUND)
            return res.status(response.code).json(response);
        } else {
            ...
            let givePlayerDataForCard = await this.playerCardData(givePlayerId, fantasyLeagueId)
            let takePlayerDataForCard = await this.playerCardData(takePlayerId, fantasyLeagueId)
...
exports.updateTradeMoveRequestStatus = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (approvedStatus == "accepted") {
            await FantasyTeamHelper.updateTradeMovePlayer(takeFlTeamId, takePlayerId, givePlayerId)
            await FantasyTeamHelper.updateTradeMovePlayer(giveFlTeamId, givePlayerId, takePlayerId)
        }
        let tradeMoveRequestDetail = await TradeMoveHelper.findTradeMoveReqById(recordId)
        let givePlayerDataForCard = await this.playerCardData(givePlayerId, fantasyLeagueId)
        let takePlayerDataForCard = await this.playerCardData(takePlayerId, fantasyLeagueId)
        ...
        //
        //send mail
        let takeTeamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(takeFlTeamId)
        ...
        let giveTeamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(giveFlTeamId)
        ...
        EmailHelper.sendEmail(giveTeamOwnerEmail, subject, message, 'tradeProposalResponse', replacements);
        //
        response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
        response.tradeMoveDetailData = tradeMoveFinalObj
        return res.status(response.code).json(response);
...
// Create Fantasy Team
exports.createFantasyTeamForUser = async (userId, fantasyLeagueId, teamSize, totalTeams) => {
    let userData = await UserHelper.foundUserById(userId)
    let userName = userData.userDetail.userName
    let teamViewName = "Team " + userName
    // let teamTitleImage = userData.profileImage
    let newFantasyTeamId = await FantasyTeamHelper.creatFantasyTeam(teamViewName, userId, fantasyLeagueId, teamSize, totalTeams)
    ...
    if (fantasyLeagueDetail != null) {
        ...
        if (fantasyLeagueDetail.flTeams.length > 0) {
            ...
            for (let i = 0; i < fantasyLeagueDetail.flTeams.length; i++) {
                ...
                let teamImage
                let teamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(fantasyTeamId)
        ...
        let flJoined = await this.fantasyLeagueJoinCheckForUser(userId, fantasyLeagueId)
        fantasyLeagueObj = {
            _id: fantasyLeagueDetail._id,
            flTitleImage: fantasyLeagueDetail.flTitleImage,
            flName: fantasyLeagueDetail.flName,
...
exports.teamMemberDetail = async (flTeamId) => {
    ...
    let newTeamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(flTeamId)
...
exports.generateFantasyLeagueSchedule = async (roundNumber, totalTeams, participatingTeamArr, fantasyLeagueId, leagueId, scheduleType) => {
    let srNo
    let teamOneArr = []
    let teamTwoArr = []
    if (roundNumber == 1) {
        ...
        let firstHalfOfTeamArray = participatingTeamArr.splice(0, centerOfTeamArray);
        let secondHalfOfTeamArray = participatingTeamArr.splice(-centerOfTeamArray);
        teamOneArr = this.shuffleFLArr(firstHalfOfTeamArray);
        teamTwoArr = this.shuffleFLArr(secondHalfOfTeamArray);
    }
    if (roundNumber > 1) {
...
exports.fantasyLeagueScheduleData = async (fantasyLeagueId) => {
    ...
    let leagueMatches = await FantasyLeagueHelper.checkLeagueSchedule(fantasyLeagueId)
    ...
    for (let k = 0; k < maxRoundCount; k++) {
        for (let l = 0; l < leagueMatches.length; l++) {
            if (leagueMatches[l].roundNumber == k + 1) {
                let teamOneName = await this.getFantasyTeamName(leagueMatches[l].teamOne)
                let teamTwoName = await this.getFantasyTeamName(leagueMatches[l].teamTwo)
                ...
                winnerTeamName = await this.getFantasyTeamName(winnerTeamId)
                // } else {
                //     winnerTeamName = ""
                // }
                let leagueMatchObj = {
                    _id: leagueMatches[l]._id,
                    srNo: leagueMatches[l].rowNumber,
                ...
                scheduleTypeForData = leagueMatches[l].scheduleType
...
exports.getFantasyTeamName = async (fantasyTeamId) => {
    ...
    if (fantasyTeamId != null) {
        let teamDeatil = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(fantasyTeamId)
        if (teamDeatil != null) {
...
exports.scheduleForFourTeams = async (teamArr, maxRoundNumber, fantasyLeagueId, leagueId) => {
    let roundNumber = parseInt(maxRoundNumber) + 1
    let checkFanatasyLeagueScheduleExist = await FantasyLeagueHelper.findFlScheduleByFantasyLeagueId(fantasyLeagueId)
    if (checkFanatasyLeagueScheduleExist.length > 0) {
        let checkFantasyLeagueCompleteAndWinnerByPlayoff = await FantasyLeagueHelper.checkAlreadyLeagueScheduleForPlayoff(fantasyLeagueId, roundNumber)
...
exports.scheduleForTwoTeams = async (maxRoundNumber, fantasyLeagueId, leagueId) => {
    console.log("2 schedule")
    let firstTeam
    let secondTeam
    let matchOneData = await FantasyLeagueHelper.getPlayOffMatchDataWithRoundNumber(fantasyLeagueId, maxRoundNumber, 1)//1 is row number(match number)
    ...
    let matchTwoData = await FantasyLeagueHelper.getPlayOffMatchDataWithRoundNumber(fantasyLeagueId, maxRoundNumber, 2)//2 is row number(match number)
...
exports.scheduleForOneTeam = async (maxRoundNumber, previousRoundWinnerTeamsArr, fantasyLeagueId, leagueId) => {
    let checkFinalScheduleRequired = await FantasyLeagueHelper.findLeagueScheduleDetailByRoundNumber(maxRoundNumber, fantasyLeagueId)
    if (checkFinalScheduleRequired.scheduleType != "final") {
        console.log("not final exist")
        let oldRoundNumber = parseInt(maxRoundNumber) - 1
        let previousMatchDataToGetFirstQualifyTeam = await FantasyLeagueHelper.getPlayOffMatchDataWithRoundNumber(fantasyLeagueId, oldRoundNumber, 1)
...
exports.leaderboardDataDetail = async (fantasyLeagueId) => {
    ...
    if (fantasyLeagueDetail != null) {
        ...
        for (let i = 0; i < flTeams.length; i++) {
            let flTeamId = flTeams[i]
            let allFlTeamPointsArr = await FantasyLeagueHelper.getAllFlTeamPointsList(fantasyLeagueId, flTeamId)
            let pointsArr = []
            ...
            teamPointsArr.push({flTeamId: flTeamId, teamTotalPoints: teamSumPoints})
        }
        if (teamPointsArr.length > 0) {
            sortedTeamPointsArr = teamPointsArr.sort((a, b) => b.teamTotalPoints - a.teamTotalPoints)
        }
        if (sortedTeamPointsArr.length > 0) {
            for (let k = 0; k < sortedTeamPointsArr.length; k++) {
                ...
                let newFlTeamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(flTeamIdForDetail)
...
exports.getFantasyLeaguePlayersList = async (fantasyLeagueId) => {
    ...
    if (fantasyLeagueDetail != null) {
        ...
        if (flTeams.length > 0) {
            for (let i = 0; i < flTeams.length; i++) {
                let flTeamId = flTeams[i]
                let fantasyTeamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(flTeamId)
...
exports.teamDetailWithPoints = async (flTeamId, leagueId, fantasyLeagueId, maxRoundNumberFromFlSchedule) => {
    let teamPlayerFinalArr = []
    let finalTeamArr = []
    let flTeamData = await FantasyTeamHelper.findFantasyTeamDetailByFantasyTeamId(flTeamId)
    if (flTeamData != null) {
        ...
        if (flTeamMembers.length > 0) {
            ...
            for (let i = 0; i < flTeamMembers.length; i++) {
                ...
                let playerLeaguePointsArr = await FantasyLeagueHelper.getPlayerPointsByLeagueIdAndMaxRoundNumber(playerId, leagueId, maxRoundNumberFromFlSchedule)
...
exports.fantasyTeamDetail = async (userId, flTeamId) => {
    ...
    if (flTeamDetail == null) {
        ...
        let fantasyLeagueDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithoutDelete(fantasyLeagueId)
        if (fantasyLeagueDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.FANTASY_LEAGUE_NOT_EXIST)
            return res.status(response.code).json(response);
        } else {
            leagueId = fantasyLeagueDetail.league
        }
        let finalTeamArr = await this.teamDetailWithPoints(flTeamId, leagueId, fantasyLeagueId, maxRoundNumberFromFlSchedule)
        if (finalTeamArr[0].teamData.length > 0) {
            ...
            for (let i = 0; i < finalTeamArr[0].teamData.length; i++) {
                ...
                let givePlayerRequest = await TradeMoveHelper.findGivePlayerForThisPlayer(fantasyLeagueId, playerIdForCheck)
                let takePlayerRequest = await TradeMoveHelper.findTakePlayerForThisPlayer(fantasyLeagueId, playerIdForCheck)
                if (givePlayerRequest != null || takePlayerRequest != null) {
                    tradeStatus = true
                } else {
                    tradeStatus = false
...
exports.playerCardData = async (playerId, fantasyLeagueId) => {
    ...
    if (flDetail == null) {
        ...
        leagueId = flDetail.league
        fantasyTeamDetail = await FantasyTeamHelper.findFantasyTeamDetailByFlId(fantasyLeagueId, playerId)
    }
    if (fantasyTeamDetail != null) {
