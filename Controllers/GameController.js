...
exports.addGame = async (req, res, next) => {
    ...
    if (!request.gameName || !request.platforms) {
        fs.unlinkSync(imagePath)
        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.MISSING_PARAMETRE);
        return res.status(response.code).json(response);
    }
    ...
    if (game != null) {
        fs.unlinkSync(imagePath)
        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.GAME_EXIST);
        return res.status(response.code).json(response);
    }
    if (game == null) {
        let gameId = await GameHelper.addGame(
            request,
            imagePath
        );
...
exports.editGame = async (req, res, next) => {
    ...
    if (gameDetail == null) {
        ...
        let response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.GAME_DOESNOT_EXIST);
        return res.status(response.code).json(response);
    } else {
        if (!req.file) {
...
exports.gameById = async (req, res) => {
    ...
    let result = await GameHelper.findGameById(gameId)
    response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
...
exports.games = async (req, res) => {
    ...
    if (req.query.query) {
        if (req.query.query.toLowerCase() == "ladder") {
            ...
            let filterGameId = [...new Set(getLadderGameToPlay.map(item => item.gameToPlay.toString()))];
        ...
        if (req.query.query.toLowerCase() == "tournament") {
            let getTournamentGameById = await TournamentHelper.getAllTournamentForGame()
            let filterGameId = [...new Set(getTournamentGameById.map(item => item.gameToPlay.toString()))];
            if (filterGameId.length > 0) {
                for (let i = 0; i < filterGameId.length; i++) {
                    let gameId = filterGameId[i]
                    let gameDetail = await GameHelper.findUserGameById(gameId)
    ...
    //         }
    //         gameResult = await GameHelper.findGameByPlatform(gamePlatform)
    //     }
    //     if (platform == 'all') {
    //         gameResult = await GameHelper.allGames()
    //     }
    //     response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
    //     response.gameData = gameResult
    //     return res.status(response.code).json(response)
    // }
}
