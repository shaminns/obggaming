...
exports.createMatchInvitation = async (req, res, next) => {
    ...
    if (userId != null) {
        if (!request.matchName || !request.gameToPlay || !request.platform
            || !request.startingDateAndTime || !request.prize) {
            fs.unlinkSync(imagePath);
            response = ResponseHelper.setResponse(
                ResponseCode.NOT_SUCCESS,
        ...
        if (request.challengeTo != "") {
            matchDetail = await MatchHelper.findMatchByTitleAndUserId(request.matchName.toLowerCase(), userId, request.challengeTo);
        }
        if (request.challengeTo == "" || !request.challengeTo) {
            matchDetail = await MatchHelper.findPublicMatchByTitleAndUserId(request.matchName.toLowerCase(), userId);
        }
        if (matchDetail != null) {
            fs.unlinkSync(imagePath);
            response = ResponseHelper.setResponse(
        ...
        if (matchDetail == null) {
            if (request.challengeTo == "" || !request.challengeTo) {
                matchId = await MatchHelper.createPublicMatchInvitation(request, userId, imagePath, startDateAndTime);
            } else {
                matchId = await MatchHelper.createMatchInvitation(request, userId, imagePath, startDateAndTime);
            }
            let prizePool = userCredits - prizeDeduction
            await UserHelper.updateCredit(userId, prizePool)
            let matchInvitation = await MatchHelper.findMatchInvitationByIdWithPopulate(matchId)
            ...
            if (request.challengeTo != "") {
                matchObj = {
                    ...
                    startDate: moment(matchInvitation.startingDateAndTime).format('MMM DD yyyy'),
                    startTime: moment(matchInvitation.startingDateAndTime).format('hh:mm A'),
...
exports.responseMatchInvitation = async (req, res) => {
    ...
    if (userId != null) {
        let userData = await UserHelper.foundUserById(userId)
        let userCredits = userData.userDetail.credits
        let matchDetail = await MatchHelper.findMatchInvitationByIdWithPopulate(request.matchId)
        ...
        if (challengeById == userId) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_ELIGIBLE_TO_ACCEPT)
            return res.status(response.code).json(response);
        }
        if (responseStatus === 'cancelled') {
            let challengeByUpdatedCredit = parseFloat(challengeByCredits) + parseFloat(prizeDeduction)
            await UserHelper.updateCredit(challengeById, challengeByUpdatedCredit)
            await MatchHelper.matchInvitationResponse(request.matchId, request.status)
        ...
        if (responseStatus === 'accepted') {
            ...
            if (userCredits >= prizeDeduction) {
                let challengeToUpdatedCredit = parseFloat(userCredits) - parseFloat(prizeDeduction)
                await UserHelper.updateCredit(userId, challengeToUpdatedCredit)
                //for public match
                if (challengeToData.challengeTo == null) {
                    await MatchHelper.publicMatchInvitationResponse(request.matchId, userId, request.status)
            ...
            if (matchData[i].winningUser != null) {
                ...
                if (challengeByMatchResult != null) {
                    for (let i = 0; i < challengeByMatchResult.results.length; i++) {
                        if (challengeByMatchResult.results[i].playerId.toString() == challengeById.toString()) {
                            challengeByMatchFinalResult = challengeByMatchResult.results[i].result
                        }
                ...
                if (challengeToMatchResult != null) {
                    for (let i = 0; i < challengeToMatchResult.results.length; i++) {
                        if (challengeToMatchResult.results[i].playerId.toString() == challengeToId.toString()) {
                            challengeToMatchFinalResult = challengeToMatchResult.results[i].result
                        }
...
exports.showMatchById = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let matchDetail = await MatchHelper.findMatchByIdPopulatedWithoutDel(matchId)
        let challengeById = matchDetail.challengeBy._id
        let challengeToId = matchDetail.challengeTo._id
        let challengeByResult
        let challengeToResult
        let matchRecordOfChallengeByUser = await this.matchDataByUserId(challengeById)
        let matchRecordOfChallengeToUser = await this.matchDataByUserId(challengeToId)
        let challengeByMatchResult = await MatchResultHelper.findUserMatchResultByMatchId(challengeById, matchId)
        let challengeByMatchFinalResult
        let challengeToMatchFinalResult
        if (challengeByMatchResult != null) {
...
exports.addMatchResult = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let matchDetail = await MatchHelper.findMatchByIdPopulatedWithoutDel(request.matchId)
        ...
        let matchResultRecord = await MatchResultHelper.findMatchResultByMatchId(request.matchId)
        if (matchResultRecord == null) {
            let matchResultId = await MatchResultHelper.addNewResult(request.matchId, matchName, gameToPlay, results)//(request, matchName, userId, gameToPlay, filePath)
            let matchData = await MatchResultHelper.findMatchByIdWithoutDel(matchResultId)
            response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
            response.matchData = matchData
            return res.status(response.code).json(response);
        }
        if (matchResultRecord != null) {
            let matchResultCheck = await MatchResultHelper.checkAlreadySubmitMatchResult(request.matchId, userId)
            ...
            if (matchResultCheck == null) {
                let matchResultId = await MatchResultHelper.addResult(request.matchId, results)
                let matchData = await MatchResultHelper.findMatchByIdWithoutDel(matchResultId)
                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                response.matchData = matchData
                return res.status(response.code).json(response);
...
exports.showAndSearchMatchForResult = async (req, res) => {
    ...
    if (!req.query.query) {
        matchData = await MatchResultHelper.allMatchWithPaginationWithoutDel(pageNo)
    }
    if (req.query.query) {
        matchData = await MatchResultHelper.findMatchByMatchNameWithPaginationWithoutDel(req.query.query.toLowerCase(), pageNo)
    }
...
exports.updateResultForMatch = async (req, res) => {
    ...
    let matchResultDetail = await MatchResultHelper.findMatchResultByIdWithDel(matchResultId)
    ...
    if (matchResultDetail != null) {
        ...
        if (resultStatus == "loss") {
            let resultArr = []
            let alreadyWinLossMatch = await MatchResultHelper.checkAlreadyWinLossMatchResult(matchId, userId)
            ...
            if (resultArr.includes("loss") == false) {
                let checkAlreadyMatchWinningUser = await MatchHelper.findAlreadyWinningUser(matchId, userId)
                ...
                await MatchResultHelper.updateMatchResult(matchResultId, userId, resultStatus)
            }
        }
        if (resultStatus == "win") {
            let resultArr = []
            ...
            if (resultArr.includes("win") == false) {
                let findWinningResult = await MatchResultHelper.findAlreadyWinMatchResult(matchId)
                if (findWinningResult != null) {
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_WIN_MATCH_RESULT)
                    return res.status(response.code).json(response);
                }
                if (findWinningResult == null) {
                    await MatchResultHelper.updateMatchResult(matchResultId, userId, resultStatus)
        ...
        let winPlayerId
        let checkPendingMatch = await MatchResultHelper.checkPendingForMatchResult(matchId, userId)
        ...
        let matchForResult = await MatchResultHelper.findMatchResultByIdWithDel(matchResultId)
        let matchDataObj = {
...
exports.deleteMatchResult = async (req, res) => {
    ...
    for (let i = 0; i < resultIdArr.length; i++) {
        ...
        let matchResultDetailById = await MatchResultHelper.findMatchResultByIdWithoutPopulate(resultId)
        console.log("match result : ", matchResultDetailById)
        if (matchResultDetailById == null) {
            notResultId.push(resultId)
        }
        if (matchResultDetailById != null) {
            let checkPendingResultExist = await MatchResultHelper.checkPendingResultExist(resultId)
...
exports.matchAutoExpire = async (req, res) => {
    ...
    for (let i = 0; i < pendingMatchList.length; i++) {
        ...
        let matchDetail = await MatchHelper.findMatchInvitationByIdWithPopulate(matchId)
