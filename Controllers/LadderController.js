...
exports.createLadder = async (req, res) => {
    ...
    if (request.totalTeams < 8) {
        fs.unlinkSync(imagePath)
        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TOTAL_TEAMS_NOT_LESS_THAN_8);
        return res.status(response.code).json(response);
    }
    if (ladderNameCheck == null) {
        let savedLadderId = await LadderHelper.createLadder(
            request, imagePath, startDateAndTime, endDateAndTime
        );
        let ladder = await LadderHelper.findLadderByIdWithoutPopulateGame(savedLadderId)
...
exports.listLadder = async (req, res) => {
    ...
    if (req.query.query) {
        result = await LadderHelper.laddersByNameWithPagination(req.query.query.toLowerCase(), pageNo);
    }
    if (!req.query.query) {
    ...
    response.ladderData = {...pagination, data: result.data}
    return res.status(response.code).json(response);
};
exports.updateLadder = async (req, res) => {
    ...
    if (ladderDetail == null) {
        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LADDER_DOES_NOT_EXIST);
        return res.status(response.code).json(response);
    } else {
        ...
        if (ladder == null) {
            ...
            let response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LADDER_DOES_NOT_EXIST);
            return res.status(response.code).json(response);
        }
        if (participatingTeams.length > 0) {
            ...
            let ladderData = await LadderHelper.findLadderByIdWithoutPopulate(request._id)
            response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL);
...
exports.deleteLadder = async (req, res) => {
    ...
    for (let i = 0; i < request.ids.length; i++) {
        ...
        if (ladderId == null) {
            ...
            let participatingTeams = ladderDetail.participatingTeams
            if (participatingTeams.length > 0) {
                let checkResultExist = await LadderResultHelper.findLadderResultExist(ladderId)
                if (checkResultExist.length == 0) {
                    ...
                    let checkPendingTournamentResults = await LadderResultHelper.findPendingResultByLadderId(ladderId)
                    if (checkPendingTournamentResults.length > 0) {
                        ...
                        if (checkResultExist.length < participatingTeams.length) {
                            ...
                            if (filterTeamArr.length != participatingTeams.length) {
                                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.CAN_NOT_DELETE_TEAM_PENDING_RESULT);
                                return res.status(response.code).json(response);
                            } else {
...
exports.joinLadder = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let participateInTeam = await TeamHelper.findParticipateInTeam(request.teamId, userId)
        if (participateInTeam == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_TEAM_MEMBER)
            return res.status(response.code).json(response);
        }
        let ladderDetail = await LadderHelper.findLadderById(request.ladderId);
        if (ladderDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LADDER_DOESNOT_EXIST);
            return res.status(response.code).json(response);
        } else {
            ...
            if (dateCheckResult) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.CAN_NOT_JOIN_AFTER_DATE)
                return res.status(response.code).json(response);
            } else {
                ...
                for (let i = 0; i < participatingMemberArr.length; i++) {
                    ...
                    for (let k = 0; k < teamDetail.teamMembers.length; k++) {
                        let teamMemberCheck = teamMemberArr.includes(teamData.teamMembers[k].userId.toString())
                ...
                await LadderHelper.addTeam(request.ladderId, request.teamId, ladderTeamResult);
...
exports.addLadderResult = async (req, res) => {
    ...
    if (!req.file) {
        response = ResponseHelper.setResponse(
            ResponseCode.NOT_SUCCESS, Message.VIDEO_IMAGE_NOT_READ);
        return res.status(response.code).json(response);
    }
    if (req.file) {
    ...
    if (userId != null) {
        ...
        if (!startDateCHeckResult) {
            ...
            if (!dateCheckResult) {
                fs.unlinkSync(filePath)
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.CAN_NOT_SUBMIT_RESULT)
                return res.status(response.code).json(response);
            } else {
                ...
                if (teamCheck == true) {
                    ...
                    let resultLadderId = await LadderResultHelper.submitResult(
                        ...
                        filePath)
                    response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                    response.ladderData = await this.ladderDetailData(resultLadderId, userId)
                    return res.status(response.code).json(response);
                }
                // }
            }
...
exports.showLaddersForResult = async (req, res) => {
    ...
    if (!req.query.query) {
        laddersForResult = await LadderResultHelper.laddersResultWithPagination(pageNo)
    }
    if (req.query.query) {
        laddersForResult = await LadderResultHelper.laddersResultByNameWithPagination(ladderName, pageNo)
    }
...
exports.updateResultForLadder = async (req, res) => {
    ...
    let ladderResultDetail = await LadderResultHelper.findLadderResultByIdForResult(ladderResultId)
    if (ladderResultDetail == null) {
        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_UPDATED)
        return res.status(response.code).json(response);
    }
    if (ladderResultDetail != null) {
        ...
        await LadderHelper.updateLadderTeamResult(ladderId, teamId, newladderTeamResult)
        let laddersForResult = await LadderResultHelper.findLadderResultById(ladderResultId)
        response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
...
exports.deleteLadderResultRecord = async (req, res) => {
    ...
    for (let i = 0; i < resultIdArr.length; i++) {
        ...
        let ladderResultDetailById = await LadderResultHelper.findLadderResultByIdForResult(resultId)
        ...
        for (let i = 0; i < teamData.length; i++) {
            ...
            if (ladderData.length != 0) {
                for (let j = 0; j < ladderData.length; j++) {
                    let ladderDate = moment(ladderData[j].startingDateAndTime).format('MMM DD yyyy');
                    let ladderTime = moment(ladderData[j].startingDateAndTime).format('hh:mm A');
                    ladderObj = {
...
exports.createAndGetTotalWarLadder = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let gameName = ""
        let ladderId = request.ladderId
        let ladderDetail = await LadderHelper.findLadderByIdWithoutPopulate(ladderId)
        if (ladderDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LADDER_NOT_EXIST)
            return res.status(response.code).json(response);
        } else {
            let gameToPlay = ladderDetail.gameToPlay
            let gameDetail = await GameHelper.findUserFranchiseGameByIdWithDeleted(gameToPlay)
            if (gameDetail == null) {
            ...
            if (!dateCheckResult) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.CAN_NOT_START_TWL)
                return res.status(response.code).json(response);
            } else {
                let checkPendingLadderResultExist = await LadderResultHelper.findPendingResultByLadderId(ladderId)
                if (checkPendingLadderResultExist.length > 0) {
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LADDER_RESULT_NOT_FINALIZE_PENDING)
                    return res.status(response.code).json(response);
                } else {
                    ...
                    if (ladderParticipatingTeams.length > 0) {
                        if (ladderParticipatingTeams.length < 8) {
                            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LESS_THAN_REQUIRED_TEAMS)
                            return res.status(response.code).json(response);
                        } else {
                            firstEigthResultArr = ladderParticipatingTeams.slice(0, 8)
                            let checkTotalWarLadderExist = await TotalWarLadderHelper.findTotalWarLadderByLadderIdWithoutDelete(ladderId)
                            if (checkTotalWarLadderExist != null) {
                                let matchResultArr = []
                                let win
                                let loss
                                let twlDetail = await TotalWarLadderHelper.findTotalWarLadderByLadderIdWithoutDelete(ladderId)
                                if (twlDetail == null) {
                                ...
                                let totalWarLadderId = await TotalWarLadderHelper.createTotalWarLadder(ladderId, gameToPlay, gameName, firstEigthResultArr)
                                //let totalWarLadderId = "62ce9903572cbc47384eec42"
                                // console.log("total war ladder : ", totalWarLadderId)
                                let twlMatchExist = await TotalWarLadderHelper.findTotalWarLadderMatchByTwlId(totalWarLadderId)
                                if (twlMatchExist.length == 0) {
                                    let totalWarLadderDetail = await TotalWarLadderHelper.findTotalWarLadderById(totalWarLadderId)
                                    let totalWarLadderParticipatingTeams = totalWarLadderDetail.participatingTeams
                                    let firstTeamsArr = totalWarLadderParticipatingTeams.slice(0, 4)
                                    let secondTeamsArr = totalWarLadderParticipatingTeams.slice(4, 8)
                                    ...
                                    for (let i = 0; i < firstTeamsArr.length; i++) {
                                        ...
                                        await TotalWarLadderHelper.createTotalWarLadderMatch(ladderId, totalWarLadderId,
                                            matchId, firstTeamsArr[i], reverseSecondTeamsArr[i])
                                    }
                                }
                                let twlMatchesArr = await TotalWarLadderHelper.findTotalWarLadderMatchByTwlId(totalWarLadderId)
...
exports.submitTotalWarLadderResult = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let checkTeamParticipation = await TotalWarLadderHelper.findTeamPariticipated(totalWarLadderId, teamId)
        if (checkTeamParticipation == null) {
            fs.unlinkSync(filePath);
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TEAM_NOT_PARTICIPATING)
            return res.status(response.code).json(response);
        } else {
            let checkIsUserTeam = await TeamHelper.findParticipateInTeam(teamId, userId)
            if (checkIsUserTeam == null) {
                fs.unlinkSync(filePath);
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_TEAM_MEMBER)
                return res.status(response.code).json(response);
            } else {
                let checkAlreadyResultSubmitted = await TotalWarLadderHelper.findAlreadyResultSubmittedByTeam(totalWarLadderId, teamId)
                if (checkAlreadyResultSubmitted != null) {
                    fs.unlinkSync(filePath);
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.RESULT_ALREADY_SUBMITTED)
                    return res.status(response.code).json(response);
                } else {
                    let twlDetail = await TotalWarLadderHelper.findTotalWarLadderById(totalWarLadderId)
                    if (twlDetail == null) {
                        fs.unlinkSync(filePath);
                        response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TWL_NOT_EXIST)
                        return res.status(response.code).json(response);
                    } else {
                        let ladderName = ""
                        let gameName = ""
                        ...
                        let teamMatchDetailForMatchId = await TotalWarLadderHelper.findTeamMatchByTeamId(totalWarLadderId, teamId)
                        if (teamMatchDetailForMatchId != null) {
                            matchId = teamMatchDetailForMatchId.matchId
                        }
                        await TotalWarLadderHelper.submitResult(totalWarLadderId, teamId, score, ladderId,
                            ladderName, gameToPlay, gameName, teamName, matchId, userId, filePath)
                    }
                    let twlData = await this.getTotalWarObj(ladderId, userId)
                    response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                    response.totalWarLadderData = twlData
...
exports.showAndSearchTotalWarLadderResults = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (req.query.query) {
            allTotalWarLadderResults = await TotalWarLadderHelper.searchAllTotalWarLadderResultWithoutDeleteWithPagination(req.query.query.toLowerCase(), pageNo)
        }
        if (!req.query.query) {
            allTotalWarLadderResults = await TotalWarLadderHelper.getAllTotalWarLadderResultWithoutDeleteWithPagination(pageNo)
        }
...
exports.deleteTotalWarLadderResults = async (req, res) => {
    ...
    if (userId != null) {
        ...
        for (let i = 0; i < resultIdArr.length; i++) {
            resultId = resultIdArr[i]
            let totalWarLadderResultById = await TotalWarLadderHelper.findTwlResultByResultId(resultId)
...
exports.updateTotalWarLadderResults = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let resultStatus = request.resultStatus.toLowerCase()
        let checkTwlResultExist = await TotalWarLadderHelper.findTwlResultByResultId(resultId)
        if (checkTwlResultExist == null) {
            ...
            let teamDetailFromTwlResultForMatchId = await TotalWarLadderHelper.findTeamMatchByTeamId(twlId, teamId)
            let matchId = teamDetailFromTwlResultForMatchId.matchId
            let checkAlreadyResultStatus = await TotalWarLadderHelper.findAlreadyResultStatusByMatchId(matchId, resultStatus)
            if (checkAlreadyResultStatus != null) {
                ...
                let recordDetailByResultId = await TotalWarLadderHelper.findTwlResultByResultId(resultId)
                let twlResultObj = {
...
exports.ladderDetailData = async (ladderId, userId) => {
    ...
    const intersection = participatingTeamsInLadderArr.filter(element => myTeamsArr.includes(element));
    ...
    for (let k = 0; k < ladderDetail.participatingTeams.length; k++) {
        let streakArr = ladderDetail.participatingTeams[k].ladderTeamResult.streakRecord
        let streakResultArr = []
        ...
        let lossCount = ladderDetail.participatingTeams[k].ladderTeamResult.lossCount
...
exports.getTotalWarObj = async (ladderId, userId) => {
    ...
    let matchResultArr = []
    let firstEigthResultArr = []
    let twlDetail = await TotalWarLadderHelper.findTotalWarLadderByLadderIdWithDelete(ladderId)
    ...
    let twlResultsArr = await TotalWarLadderHelper.getAllTotalWarLadderResultByTwlId(totalWarLadderId)
    ...
    if (twlResultsArr.length == 0) {
        ...
        if (totalWarLadderParticipatingTeams.length > 0) {
            ...
            for (let i = 0; i < twlDetail.participatingTeams.length; i++) {
                ...
                let checkTwlResultByTeamId = await TotalWarLadderHelper.findResultById(totalWarLadderId, teamId)
    ...
    let checkResultSubmit = await this.checkUserSubmitTwlResult(ladderId, userId)
    return {resultSubmit: checkResultSubmit, matches: totalWarLadderData, matchResults: matchResultArr}
}
exports.checkUserSubmitTwlResult = async (ladderId, userId) => {
    let resultSubmitted = false
    let twlDetail = await TotalWarLadderHelper.findTotalWarLadderByLadderIdWithoutDelete(ladderId)
    let totalWarLadderId = twlDetail._id
    let twlResultsArr = await TotalWarLadderHelper.getAllTotalWarLadderResultByTwlIdWithoutDelete(totalWarLadderId)
    if (twlResultsArr.length == 0) {
