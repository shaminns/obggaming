...
exports.addTryoutRequest = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let checkAlreadySentRequest = await TryoutHelper.findSentRequest(request.franchiseId, userId)
        ...
        if (checkAlreadySentRequest == null) {
            let franchiseDetail = await FranchiseHelper.findFranchiseByIdWithoutDelete(request.franchiseId)
            let franchiseOwnerId = franchiseDetail.createdBy
            let requestDateAndTime = await GeneralHelper.dateTimeToUtc(request.dateAndTime)
            await TryoutHelper.createTryoutRequest(request.teamId, requestDateAndTime, userId, request.franchiseId, franchiseOwnerId)
            let franchiseData = await franchiseController.franchiseDetailDataByFranchiseId(request.franchiseId, userId)
...
exports.updateTryoutRequestStatus = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let teamName = await leagueController.getTeamName(tryoutRecord.franchiseTeamId)
...
exports.addUserToFranchiseTeam = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let userFranchiseTeamDetail = await franchiseController.getUserFranchiseTeam(teamMemberId)
        if (userFranchiseTeamDetail != null) {
            userFranchiseTeamId = userFranchiseTeamDetail._id
            let userFranchiseDetail = await franchiseController.getUserUserFrachise(userFranchiseTeamId)
            if (userFranchiseDetail != null) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_IN_FRANCHISE_TEAM + ". Please leave that franchise(team) first!")
                return res.status(response.code).json(response);
            }
        } else {
            let checkAlreadyTeamMember = await TeamHelper.findParticipateInTeam(teamId, teamMemberId)
            ...
            if (checkAlreadyTeamMember == null) {
                ...
                // return res.status(response.code).json(response);
                let tryoutRecord = await TryoutHelper.findTryoutRequestById(request._id)
                let teamName = await leagueController.getTeamName(userFranchiseTeamId)
...
exports.tryoutData = async (userId, pageNo) => {
    ...
    if (franchiseData == null) {
        ...
        if (teamDetail != null) {
            ...
            let franchiseDetail = await franchiseController.getUserUserFrachise(teamId)
    ...
    tryoutRecord = await TryoutHelper.findTryoutByFranchiseIdWithoutDeleteWithPagination(franchiseId, pageNo)
    if (tryoutRecord != null) {
        if (tryoutRecord.data.length > 0) {
            for (let i = 0; i < tryoutRecord.data.length; i++) {
                let teamName = await leagueController.getTeamName(tryoutRecord.data[i].franchiseTeamId)
                let userData = await UserHelper.foundUserById(tryoutRecord.data[i].userId)
                let userName = userData.userDetail.userName
                let tryoutDate = moment(tryoutRecord.data[i].dateAndTime).format('DD MMM, yyyy');
                let tryoutTime = moment(tryoutRecord.data[i].dateAndTime).format('hh:mm A');
                let tryoutObj = {
