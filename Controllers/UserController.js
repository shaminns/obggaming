...
exports.addUser = async (req, res) => {
    ...
    let user = await UserHelper.createUser(
        email,
        password,
        fullName,
        userName
...
exports.sendFriendRequest = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (findUser != null) {
            ...
            let receivedFriendReqeustCheck = await UserHelper.checkAlreadyReceivedRequest(
                userId,
                findUser._id
            );
            if (receivedFriendReqeustCheck != null) {
                response = ResponseHelper.setResponse(
                    ResponseCode.NOT_SUCCESS,
                    Message.INVALID_REQUEST + ". " + Message.ALREADY_REQUESTED + " to you." + Message.CHECK_FRIEND_REQUEST
                );
                return res.status(response.code).json(response);
            }
            if (friendReqeustCheck == null) {
                ...
                let sentRequest = await UserHelper.sentFriendRequestToFriend(userId, findUser._id)
                let receivedRequest = await UserHelper.receivedFriendRequestToFriend(findUser._id, userId)
                if (sentRequest != null) {
        ...
        if (user == null) {
            ...
            if (findRequest == null) {
                ...
                if (alreadyAdded == false) {
                    ...
                    await UserHelper.acceptRequest(userId, fromId, request.status);
                    let result = await UserHelper.findAcceptedRequestToFriend(fromId, userId)
                    let myRequests = await this.myRequestData(userId)
                    response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL);
                    response.receivedRequests = myRequests.receivedRequests
                    response.sentRequests = myRequests.sentRequests
...
exports.deleteFriendRequest = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let checkFriendRequestExist = await UserHelper.findSentRequestToFriend(userId, friendId)
    ...
    if (userId != null) {
        ...
        if (friendsList != null) {
            for (let i = 0; i < friendsList.userDetail.friends.length; i++) {
                ...
                friendsDataArr.push({_id: friendId, fullName: fullName, userName: userName, profileImage: profileImage})
            }
        }
        response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL);
    ...
    if (userId != null) {
        if (req.query.query) {
            ...
            if (!req.query.query.match(format)) {
                ...
                let searchData = await UserHelper.findUserByUserNameAndFullName(searchValue)
...
exports.userProfileById = async (req, res) => {
    ...
    if (userId != null) {
        if (req.query.query) {
            ...
            let sentRequest = await UserHelper.sentFriendRequestToFriend(userId, searchedUserId)
            let receivedRequest = await UserHelper.receivedFriendRequestToFriend(searchedUserId, userId)
            if (sentRequest != null) {
                if (sentRequest.status == "pending") {
                    friendRequest = "sent"
                }
...
exports.myRequestData = async (userId) => {
    ...
    let receivedTeamInvitesResult = await TeamHelper.findAllReceivedTeamInvite(userId)
    let sendTeamInvitesResult = await TeamHelper.findAllSendTeamInvite(userId)
    let receivedMatchInviteResult = await MatchHelper.findAllReceivedMatchInvite(userId)
    let sendMatchInvitateResult = await MatchHelper.findAllSendMatchInvite(userId)
    let receivedTryoutRequest = await TryoutHelper.findAllReceivedTryoutRequest(userId)
    let sendTryoutRequest = await TryoutHelper.findAllSendTryoutRequest(userId)
    let receivedFlRequest = await FantasyLeagueHelper.findAllReceivedFlRequest(userId)
    ...
    if (receivedResult.length > 0) {
        for (let i = 0; i < receivedResult.length; i++) {
            ...
            let userFromDetail = await UserHelper.foundUserById(receivedResult[i].from);
    ...
    if (receivedTeamInvitesResult.length > 0) {
        for (let i = 0; i < receivedTeamInvitesResult.length; i++) {
            let userToDetail = await UserHelper.foundUserById(receivedTeamInvitesResult[i].to);
            let userToUserName = userToDetail.userDetail.userName;
            let userFromDetail = await UserHelper.foundUserById(receivedTeamInvitesResult[i].from);
            let userFromUserName = userFromDetail.userDetail.userName;
            let teamDetail = await TeamHelper.findTeamInviteByTeamId(receivedTeamInvitesResult[i].teamId)
            if (teamDetail == null) {
    ...
    if (receivedMatchInviteResult.length > 0) {
        for (let i = 0; i < receivedMatchInviteResult.length; i++) {
            let userToDetail = await UserHelper.foundUserById(receivedMatchInviteResult[i].challengeTo);
            let userToUserName = userToDetail.userDetail.userName;
            let userFromDetail = await UserHelper.foundUserById(receivedMatchInviteResult[i].challengeBy);
            let userFromUserName = userFromDetail.userDetail.userName;
    ...
    if (receivedFlRequest.length > 0) {
        for (let i = 0; i < receivedFlRequest.length; i++) {
            let userToDetail = await UserHelper.foundUserById(receivedFlRequest[i].to);
            let userToUserName = userToDetail.userDetail.userName;
            let userFromDetail = await UserHelper.foundUserById(receivedFlRequest[i].from);
            let userFromUserName = userFromDetail.userDetail.userName;
            let flname
            let flDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithDelete(receivedFlRequest[i].fantasyLeagueId)
            if (flDetail == null) {
    ...
    if (receivedTradeMoveRequest.length > 0) {
        for (let i = 0; i < receivedTradeMoveRequest.length; i++) {
            ...
            let fromUserFlTeam = await FantasyTeamHelper.findFantasyTeamDetailByFlId(fantasyLeagueId, fromUserId)
            ...
            let toUserFlTeam = await FantasyTeamHelper.findFantasyTeamDetailByFlId(fantasyLeagueId, toUserId)
    ...
    if (sendTeamInvitesResult.length > 0) {
        for (let i = 0; i < sendTeamInvitesResult.length; i++) {
            let userToDetail = await UserHelper.foundUserById(sendTeamInvitesResult[i].to);
            let userToUserName = userToDetail.userDetail.userName;
            let userFromDetail = await UserHelper.foundUserById(sendTeamInvitesResult[i].from);
            let userFromUserName = userFromDetail.userDetail.userName;
            let teamDetail = await TeamHelper.findTeamInviteByTeamId(sendTeamInvitesResult[i].teamId)
            if (teamDetail == null) {
    ...
    if (sendMatchInvitateResult.length > 0) {
        for (let i = 0; i < sendMatchInvitateResult.length; i++) {
            let userToDetail = await UserHelper.foundUserById(sendMatchInvitateResult[i].challengeTo);
            if (userToDetail != null) {
                userToUserName = userToDetail.userDetail.userName;
                let userFromDetail = await UserHelper.foundUserById(sendMatchInvitateResult[i].challengeBy);
                let userFromUserName = userFromDetail.userDetail.userName;
    ...
    if (sendTryoutRequest.length > 0) {
        for (let i = 0; i < sendTryoutRequest.length; i++) {
            let userToDetail = await UserHelper.foundUserById(sendTryoutRequest[i].franchiseOwnerId);
            let userToUserName = userToDetail.userDetail.userName;
            let userFromDetail = await UserHelper.foundUserById(sendTryoutRequest[i].userId);
            let userFromUserName = userFromDetail.userDetail.userName
    ...
    if (sendFlRequest.length > 0) {
        for (let i = 0; i < sendFlRequest.length; i++) {
            ...
            let userToUserName = userToDetail.userDetail.userName;
            let userFromDetail = await UserHelper.foundUserById(sendFlRequest[i].from);
            let userFromUserName = userFromDetail.userDetail.userName;
            let flDetail = await FantasyLeagueHelper.findFantasyLeagueByIdWithDelete(sendFlRequest[i].fantasyLeagueId)
            if (flDetail == null) {
                console.log("FL not found : ", receivedFlRequest[i].fantasyLeagueId)
            } else {
                flName = flDetail.flName
    ...
    finalReceivedRequestArr = receiveTradeMoveArr.concat(finalReceivedMatchInvArr)
    ...
    if (finalReceivedRequestArr.length > 0) {
        sortedReceivedArray = finalReceivedRequestArr.sort((a, b) => new moment(b.createdAt) - new moment(a.createdAt))
    }
    if (finalSendRequestArr.length > 0) {
        sortedSendArray = finalSendRequestArr.sort((a, b) => new moment(b.createdAt) - new moment(a.createdAt))
    }
    return {receivedRequests: sortedReceivedArray, sentRequests: sortedSendArray}
}
...
exports.userProfielDetailData = async (userId) => {
    ...
    if (matchData.length > 0) {
        if (matchData[i].winningUser != null) {
            ...
            let challengeByMatchResult = await MatchResultHelper.findUserMatchResultByMatchId(challengeById, matchId)
            if (challengeByMatchResult != null) {
                for (let j = 0; j < challengeByMatchResult.results.length; j++) {
                    if (challengeByMatchResult.results[j].playerId.toString() == challengeById.toString()) {
            ...
            let challengeToMatchResult = await MatchResultHelper.findUserMatchResultByMatchId(challengeToId, matchId)
            if (challengeToMatchResult != null) {
                for (let k = 0; k < challengeToMatchResult.results.length; k++) {
                    if (challengeToMatchResult.results[k].playerId.toString() == challengeToId.toString()) {
        ...
        matchObj = {
            ...
            startDate: moment(matchData[i].startingDateAndTime).format('MMM DD yyyy'),
    ...
    let userType = await franchiseController.getUserFranchiseModeAndFranchiseId(userId)
    let userData = {
