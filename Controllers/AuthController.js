...
exports.createUser = async (req, res, next) => {
    ...
    let userDetailByUserName = await UserHelper.foundUserByUserNameWithoutDeleted(userName)
    ...
    let userId = await UserHelper.createUser(
        password,
        userDetail,
        request
    )
...
exports.login = async (req, res, next) => {
    ...
    if (request.email) {
        resultCheck = CredentialHelper.emailCheck(request.email.toLowerCase().trim());
        if (resultCheck === false) {
            response = ResponseHelper.setResponse(
                ResponseCode.NOT_SUCCESS,
                Message.INVALID_EMAIL
...
exports.forgot = async (req, res, next) => {
    ...
    await UserHelper.updateResetForgotPasswordToken(request, forgotToken, isoTokenExpiry)
    const replacements = {
        link: 'go.onlinebattleground.com/auth/reset/' + foundUser._id, // hard code because env update issue
        appName: process.env.APP_NAME,
        mailFrom: process.env.MAIL_FROM,
    }
    EmailHelper.sendEmail(userEmail, subject, message, 'forgot-password-new', replacements);
    response = ResponseHelper.setResponse(
        ResponseCode.SUCCESS,
        Message.REQUEST_SUCCESSFUL,
    );
