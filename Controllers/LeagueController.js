...
const {getPreviousRoundWinnerList} = require("../Services/LeagueScheduleHelper");
...
const {dnsCache} = require("nodemailer/lib/shared");
...
exports.createLeague = async (req, res) => {
    ...
    if (userId != null) {
        if (!request.leagueName || !request.prize || !request.totalTeams || !request.teamSize
            || !request.startingDate || !request.endingDate || !request.gameToPlay
        ) {
            fs.unlinkSync(imagePath)
            response = ResponseHelper.setResponse(
                ResponseCode.NOT_SUCCESS,
        ...
        let startDate = moment(request.startingDate).utc().format("MMM DD YYYY hh:mm a")
        let endDate = moment(request.endingDate).utc().format("MMM DD YYYY hh:mm a")
        ...
        if (currentYear.toString() != startingYear.toString()) {
            fs.unlinkSync(imagePath)
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.INVALID_STARTING_DATE);
            return res.status(response.code).json(response);
        }
        let leagueForGameCheck = await LeagueHelper.findLeagueByGameIdAndStartingDate(request.gameToPlay)
        let leagueNewStartingDate = moment(request.startingDate).format('yyyy-MM-DD');
        ...
        let duration = moment.duration(momentLeagueNewEndingDate.diff(momentLeagueNewStartingDate));
        let days = duration.asDays();
        if (days < 1) {
            fs.unlinkSync(imagePath)
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.INVALID_ENDING_DATE);
            return res.status(response.code).json(response);
        }
        if (leagueForGameCheck.length > 0) {
            for (let i = 0; i < leagueForGameCheck.length; i++) {
                let previousLeagueStartingDate = moment(leagueForGameCheck[i].startingDate).format('yyyy');
                if (previousLeagueStartingDate.toString() == currentYear) {
                    fs.unlinkSync(imagePath)
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LEAGUE_FOR_THIS_YEAR_ALREADY_EXIST);
                    return res.status(response.code).json(response);
                }
            }
        } else {
            let leagueNameCheck = await LeagueHelper.findLeagueByNameWithoutDelete(request.leagueName.toLowerCase().trim())
            ...
            if (leagueNameCheck == null) {
                let newLeagueId = await LeagueHelper.createLeague(request, imagePath, startDate, endDate)
...
exports.getAllLeagues = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (role != "Admin") {
            response = ResponseHelper.setResponse(ResponseCode.NOT_AUTHORIZE, Message.AUTHENTICATION_FAILED)
            return res.status(response.code).json(response);
        } else {
            if (req.query.query) {
                leagueResult = await LeagueHelper.searchLeagueByNameWithPagination(req.query.query.toLowerCase(), pageNo)
            }
            if (!req.query.query) {
            ...
            if (leagueResult.data.length > 0) {
                for (let i = 0; i < leagueResult.data.length; i++) {
                    let gameDetail = await GameHelper.findGameByIdWithDelete(leagueResult.data[i].gameToPlay)
                    if (gameDetail != null) {
                        let gameName = gameDetail?.gameName
                        let leagueStartingDate = moment(leagueResult.data[i].startingDate).format('MMM DD, yyyy');
                        let leagueEndingDate = moment(leagueResult.data[i].endingDate).format('MMM DD, yyyy');
                        let leagueYear = moment(leagueResult.data[i].createdAt).format('yyyy');
                        let leagueObj = {
...
exports.editLeague = async (req, res) => {
    ...
    let pngImage
    if (!req.file) {
        let leagueDetail = await LeagueHelper.findLeagueByIdWithoutDeleteWithPopulate(request._id)
    ...
    if (userId != null) {
        ...
        if (request.leagueName) {
            let leagueNameCheck = await LeagueHelper.findLeagueByNameWithoutDelete(request.leagueName.toLowerCase().trim())
            if (leagueNameCheck != null) {
                if (leagueNameCheck.leagueName != request.leagueName.toLowerCase().trim()) {
                    if (req.file) {
                        fs.unlinkSync(imagePath)
                    }
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NAME_ALREADY_EXIST);
...
exports.showAllLeagues = async (req, res) => {
    ...
    if (userId != null) {
        ...
        for (let i = 0; i < allLeague.length; i++) {
            let leagueStartingDate = moment(allLeague[i].startingDate).format('MMM DD yyyy');
            let leagueEndingDate = moment(allLeague[i].endingDate).format('MMM DD yyyy');
...
exports.joinLeague = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let leagueDetail = await LeagueHelper.findLeagueByIdWithoutDeleteWithPopulate(request.leagueId)
        if (leagueDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LEAGUE_DOES_NOT_EXISTS);
            return res.status(response.code).json(response);
        } else {
            leagueTeamSizeRequired = leagueDetail.teamSize
        }
        let leagueTeamExist = await LeagueHelper.findTeamInLeague(request.leagueId, request.teamId)
        if (leagueTeamExist != null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.TEAM_EXIST);
            return res.status(response.code).json(response);
        }
        ...
        let franchiseDetail = await FranchiseHelper.findFranchiseIdByTeamId(request.teamId)
        if (franchiseDetail == null) {
            response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_FRANCHISE_TEAM)
            return res.status(response.code).json(response);
        } else {
            let userFranchiseTeams = franchiseDetail.franchiseTeams
            const intersection = leagueParticipatingTeams.filter(element => userFranchiseTeams.includes(element));
            if (intersection.length > 0) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.FRANCHISE_PARTICIPATING)
                return res.status(response.code).json(response);
            }
            if (intersection.length == 0) {
                ...
                let participatingInLeagueCheck = await TeamHelper.findAlreadyParticipatingInLeague(request.teamId, leagueId)
                if (participatingInLeagueCheck == null) {
                    await TeamHelper.updateLeagueDetail(request.teamId, leagueId)
                }
                let participatingGameCheck = await TeamHelper.findAlreadyParticipatingInGame(request.teamId, gameToPlay._id)
                if (participatingGameCheck == null) {
                    await TeamHelper.updateTournamentGameDetail(request.teamId, gameToPlay._id)
                }
                await UserHelper.updateCredit(userId, finalCredit)
                let teamArr = []
                let leagueJoinedStatus = await this.userLeagueJoinedStatus(userId, request.leagueId)
    ...
    if (userId != null) {
        ...
        if (userRole != "Admin") {
            ...
            for (let i = 0; i < request.leagueId.length; i++) {
                ...
                let leagueEndDate = moment(leagueDetail.endingDate).format('yyyy-MM-DD');
            ...
            if (notDeletedLeagueId.length > 0) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.SOME_LEAGUE_HAVE_PENDING_RESULT)
                return res.status(response.code).json(response);
            }
    ...
    if (userId != null) {
        ...
        if (totalTeams < totalRequiredTeams) {
            ...
            if (dateCheckResult == false) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.LEAGUE_START_DATE_WAIT)
                response.leagueScheduleData = []
                return res.status(response.code).json(response);
            } else {
                let alreadyScheduleCheck = await LeagueScheduleHelper.findLeagueScheduleByLeagueId(leagueId)
                if (alreadyScheduleCheck.length == 0) {
                    scheduleType = "round"
                    let shuffledParticipatingTeamArr = this.shuffle(participatingTeamArr);
                    await this.generateLeagueSchedule(totalTeams, shuffledParticipatingTeamArr, roundNumber, leagueId, scheduleType)
                    let leagueMatchScheduleDetail = await this.leagueScheduleData(request.leagueId)
                    response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                ...
                if (alreadyScheduleCheck.length > 0) {
                    let leagueDetailForMaxRoundNumber = await LeagueScheduleHelper.getLeagueDetailForMaxRoundNumber(leagueId)
                    let maxRoundNumber = leagueDetailForMaxRoundNumber[0].roundNumber
                    let checkAlreadyLeagueScheduleResult = await LeagueResultHelper.checkAlreadyLeagueScheduleResult(leagueId)
                    ...
                    if (checkAlreadyLeagueScheduleResult.length > 0) {
                        let checkAlreadyLeagueSchedulePendingResult = await LeagueResultHelper.checkAlreadyLeagueSchedulePendingResult(leagueId, maxRoundNumber)
                    ...
                    let checkLeagueCompleteAndWinnerByRound = await LeagueScheduleHelper.checkAlreadyLeagueSchedule(leagueId, maxRoundNumber)
                    ...
                    if (checkLeagueCompleteAndWinnerByRound.length == 0) {
                        ...
                        if (totalTeams == 4 && maxRoundNumber == 1) {
                            let firstRoundFourTeamsArr = await LeagueResultHelper.getPreviousRoundFourTeamList(leagueId, maxRoundNumber)
                            console.log("first Round 4 teams : ", firstRoundFourTeamsArr)
                            previousRoundWinnerTeamsArr = firstRoundFourTeamsArr
                        } else {
                            previousRoundWinnerTeamsArr = await LeagueResultHelper.getPreviousRoundWinnerList(leagueId, maxRoundNumber)
                        }
                        if (previousRoundWinnerTeamsArr.length == 32 || previousRoundWinnerTeamsArr.length == 16 ||
                            ...
                            for (let l = 0; l < previousRoundWinnerTeamsArr.length; l++) {
                                let teamArrForAddAllKillPoints = await LeagueResultHelper.getWinnerTeamsWithKillPoint(previousRoundWinnerTeamsArr[l].teamId, leagueId, maxRoundNumber)
                                for (let m = 0; m < teamArrForAddAllKillPoints.length; m++) {
                                    allKillPoints = parseInt(teamArrForAddAllKillPoints[m].killPoints) + parseInt(allKillPoints)
                                }
                            ...
                            console.log("final TeamArr with add kill points : ", finalTeamArrWithAddKillPoints)
                            sortedTeamArrWithKillPoints = finalTeamArrWithAddKillPoints.sort(function (a, b) {
                                return b.killPoints - a.killPoints
                            })
                            console.log("sorted final TeamArr with add kill points : ", sortedTeamArrWithKillPoints)
                            ...
                            if (previousRoundWinnerTeamsArr.length > 4) {
                                ...
                                await this.generateLeagueSchedule(totalTeams, teamArr, roundNumber, leagueId, scheduleType)
                                let leagueMatchScheduleDetail = await this.leagueScheduleData(request.leagueId)
                                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                                response.leagueScheduleData = leagueMatchScheduleDetail
                                response.message = Message.CHECK_NEW_SCHEDULE
                            ...
                            if (previousRoundWinnerTeamsArr.length == 4) {
                                ...
                                let leagueMatchScheduleDetail = await this.scheduleForFourTeams(teamArr, maxRoundNumber, leagueId)
                                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                                response.leagueScheduleData = leagueMatchScheduleDetail
                                response.message = Message.CHECK_NEW_SCHEDULE
                                return res.status(response.code).json(response);
                            ...
                            if (previousRoundWinnerTeamsArr.length == 2) {
                                ...
                                let leagueMatchScheduleDetail = await this.scheduleForTwoTeams(maxRoundNumber, leagueId)
                                response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                                response.leagueScheduleData = leagueMatchScheduleDetail
                                response.message = Message.CHECK_NEW_SCHEDULE
                                return res.status(response.code).json(response);
                            ...
                            if (previousRoundWinnerTeamsArr.length == 1) {
                                let checkLeagueFinalExist = await LeagueScheduleHelper.checkLeagueFinalExist(leagueId)
                                if (checkLeagueFinalExist == null) {
                                    console.log("1 schedule")
                                    let leagueMatchScheduleDetail = await this.scheduleForOneTeam(maxRoundNumber, previousRoundWinnerTeamsArr, leagueId)
                                    response = ResponseHelper.setResponse(ResponseCode.SUCCESS, Message.REQUEST_SUCCESSFUL)
                                    response.leagueScheduleData = leagueMatchScheduleDetail
                                    response.message = Message.CHECK_NEW_SCHEDULE
                                    return res.status(response.code).json(response);
                                ...
                                if (checkLeagueFinalExist != null) {
                                    ...
                                    let leagueDetails = await LeagueHelper.findLeagueByIdWithoutDeleteWithPopulate(request.leagueId)
                                    if (leagueDetails.winningTeam == null) {
                                        await LeagueHelper.updateWinnerTeam(request.leagueId, winnigTeamId)
                                    }
                                    let winningTeamName = await this.getTeamName(winnigTeamId)
...
exports.submitLeagueMatchResult = async (req, res) => {
    ...
    if (userId != null) {
        let franchiseTeamId
        let gameName
        if (!request.leagueId, !request.roundNumber, !request.scheduleType, !request.matchId) {
            ...
            let leagueId = request.leagueId
            let userFranchiseTeamDetail = await FranchiseController.getUserFranchiseTeam(userId);
            if (userFranchiseTeamDetail == null) {
                console.log("participating team detail not found")
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NOT_PARTICIPATING)
                return res.status(response.code).json(response);
            } else {
            ...
            let leagueData = await LeagueHelper.findLeagueByIdWithoutDeleteWithPopulate(leagueId)
            if (leagueData != null) {
                gameName = leagueData.gameToPlay.gameName
            }
            let score = request.score
            let roundNumber = request.roundNumber
            let scheduleType = request.scheduleType
            let matchId = request.matchId
            // console.log("team id : ", franchiseTeamId)
            let validateMatchId = await LeagueScheduleHelper.checkTeamAndMatchId(franchiseTeamId, matchId)
            if (validateMatchId.length == 0) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.MATCH_ID_NOT_VALID)
                return res.status(response.code).json(response);
            }
            let checkTypeAndRoundNumberIsCorrect = await LeagueScheduleHelper.findTypeAndRoundNumber(matchId)
            if (checkTypeAndRoundNumberIsCorrect == null) {
                response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.NO_RECORD_WITH_MATCH_ID)
                return res.status(response.code).json(response);
            } else {
                let recordRoundNumber = checkTypeAndRoundNumberIsCorrect.roundNumber
                console.log(recordRoundNumber, request.roundNumber)
                let recordScheduleType = checkTypeAndRoundNumberIsCorrect.scheduleType
                console.log(recordScheduleType, request.scheduleType)
                let roundResult = request.roundNumber.toString().match(recordRoundNumber.toString())
                let scheduleTypeResult = request.scheduleType.toString().match(recordScheduleType.toString())
                if (roundResult == null || scheduleTypeResult == null) {
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.SCHEDULE_TYPE_ROUND_NO_INCORRECT)
                    return res.status(response.code).json(response);
                } else {
                    let checkAlreadySubmitResult = await LeagueResultHelper.findAlreadySubmittedResult(franchiseTeamId, matchId)
                    if (checkAlreadySubmitResult != null) {
                        ...
                        await LeagueResultHelper.addLeagueResult(userId, leagueId, leagueName, franchiseTeamId,
...
exports.showLeagueResult = async (req, res) => {
    ...
    if (userId != null) {
        ...
        if (req.query.query) {
            franchiseLeagueResult = await LeagueResultHelper.findLeagueResultWithLeagueNameOrMatchIdWithPagination(req.query.query.toLowerCase(), pageNo)
        }
        if (!req.query.query) {
            franchiseLeagueResult = await LeagueResultHelper.findAllLeagueResultsWithOutDeleteWithPagination(pageNo)
        }
...
exports.updateLeagueMatchResult = async (req, res) => {
    ...
    if (userId != null) {
        ...
        let checkLeagueResultExist = await LeagueResultHelper.findLeagueResultByResultId(resultId)
        let roundNumber
        let scheduleType
        ...
        if (checkLeagueResultExist != null) {
            ...
            if (resultStatus == "loss") {
                let checkAlreadyHaveResultStatus = await LeagueResultHelper.findAlreadyWinLossOfMatchId(matchId, resultStatus)
                if (checkAlreadyHaveResultStatus != null) {
                    response = ResponseHelper.setResponse(ResponseCode.NOT_SUCCESS, Message.ALREADY_HAVE_THIS_STATUS)
                    return res.status(response.code).json(response);
                }
                if (checkAlreadyHaveResultStatus == null) {
                    let checkAlreadyPlayerResults = await LeagueResultHelper.checkAlreadyPlayerResults(request.resultId)
                    if (checkAlreadyPlayerResults.playerResults.length == 0) {
                        for (let i = 0; i < playerKillPoints.length; i++) {
                            ...
                            await LeagueResultHelper.updateLeagueMatchResultPlayerPoints(request.resultId, playerId, userKillPoints)
                            killPoints = (parseInt(killPoints) + parseInt(userKillPoints)).toFixed(0)
                            await UserHelper.updateUserPoints(playerId, killPoints)
                            await LeagueResultHelper.addPlayerLeaguePoints(playerId, userKillPoints, leagueIdForResult, scheduleType, roundNumber)
                        }
                    } else {
                        ...
                        if (playerResults.length > 0) {
                            for (let m = 0; m < playerResults.length; m++) {
                                ...
                                let userPts
                                let userFinalPts
                                let userFinalPoints = (parseInt(userPoints) - parseInt(killPoints)).toFixed(0)
                                await UserHelper.updateUserPoints(playerId, userFinalPoints)
                            }
                        }
                        ...
                        for (let i = 0; i < playerKillPoints.length; i++) {
                            ...
                            await LeagueResultHelper.deletePlayerLeaguePoints(playerId, leagueIdForResult, scheduleType, roundNumber)
                            await LeagueResultHelper.addPlayerLeaguePoints(playerId, userKillPoints, leagueIdForResult, scheduleType, roundNumber)
                        }
                    }
                    await LeagueResultHelper.updateLeagueMatchResult(resultId, resultStatus, killPoints, placePoints)
                }
            }
            if (resultStatus == "win") {
                let checkAlreadyHaveResultStatus = await LeagueResultHelper.findAlreadyWinLossOfMatchId(matchId, resultStatus)
            ...
            let franchiseLeagueResult = await LeagueResultHelper.findLeagueResultByResultId(resultId)
            let teamViewName = await this.getTeamName(franchiseLeagueResult.teamId)
            let teamMembersFinalArr = []
            let teamData = await TeamHelper.findTeamDeatilByTeamId(franchiseLeagueResult.teamId)
            ...
            let leagueResultObj = {
                ...
                total: parseInt((franchiseLeagueResult.killPoints) + parseInt(franchiseLeagueResult.placePoints)),
...
exports.leagueStanding = async (req, res) => {
    ...
    if (userId != null) {
        ...
        for (let i = 0; i < leagueParticipatingTeams.length; i++) {
            let teamName = await this.getTeamName(leagueParticipatingTeams[i])
            let teamWinDetail = await LeagueResultHelper.findTeamWinDetail(leagueId, leagueParticipatingTeams[i])
            if (teamWinDetail != null) {
                winCount = teamWinDetail.length
            }
            let teamLossDetail = await LeagueResultHelper.findTeamLossDetail(leagueId, leagueParticipatingTeams[i])
    ...
    if (userId != null) {
        ...
        if (participatingTeams.length > 0) {
            for (let i = 0; i < participatingTeams.length; i++) {
                ...
                let teamName = await this.getTeamName(teamId)
                let teamDetailInLeague = await LeagueResultHelper.findTeamDetailInLeague(leagueId, teamId)
...
exports.deleteLeagueResult = async (req, res) => {
    ...
    for (let i = 0; i < resultIdArr.length; i++) {
        ...
        let leagueResultDetailById = await LeagueResultHelper.findLeagueResultByResultId(resultId)
...
exports.leagueDetailData = async (leagueId, userId) => {
    ...
    if (franchiseIdArr.length > 0) {
        ...
        //     teamId = teamId
        // }
        let leagueJoinedStatus = await this.userLeagueJoinedStatus(userId, leagueId)
        let leagueStartingDate = moment(leagueDetail.startingDate).format('MMM DD yyyy');
        let leagueEndingDate = moment(leagueDetail.endingDate).format('MMM DD yyyy');
        let leagueObj = {
        ...
        if (leagueTeamsArr.length > 0) {
            for (let i = 0; i < leagueTeamsArr.length; i++) {
                ...
                let leaderDetail = await UserHelper.foundUserById(teamDetail.teamLeader)
...
exports.leagueObjDataWithoutTeams = async (leagueId) => {
    let leagueJoined
    let leagueDetail = await LeagueHelper.findLeagueByIdWithoutDeleteWithPopulate(leagueId)
    let leagueStartingDate = moment(leagueDetail.startingDate).local().format('MMM DD yyyy');
    let leagueEndingDate = moment(leagueDetail.endingDate).utc().format('MMM DD yyyy');
...
exports.generateLeagueSchedule = async (totalTeams, participatingTeamArr, roundNumber, leagueId, scheduleType) => {
    let srNo
    let teamOneArr = []
    let teamTwoArr = []
    if (roundNumber == 1) {
        ...
        teamOneArr = this.shuffle(firstHalfOfTeamArray);
        teamTwoArr = this.shuffle(secondHalfOfTeamArray);
    }
    if (roundNumber > 1) {
    ...
    for (let k = 0; k < maxRoundCount; k++) {
        for (let l = 0; l < laegueMatches.length; l++) {
            if (laegueMatches[l].roundNumber == k + 1) {
                let teamOneName = await this.getTeamName(laegueMatches[l].teamOne)
                let teamTwoName = await this.getTeamName(laegueMatches[l].teamTwo)
                ...
                let leagueTotalResultByMatchId = await LeagueResultHelper.findTotalResultByMatchId(matchId)
                let totalSubmittedResult = leagueTotalResultByMatchId.length
                if (leagueResutlDetailByMatchId.length == 0 && totalSubmittedResult == 2) {
                    winnerTeamName = await this.getTeamName(winnerTeamId)
                } else {
                    winnerTeamName = ""
                }
                let leagueMatchObj = {
...
exports.leagueResultData = async (franchiseLeagueResult) => {
    ...
    if (franchiseLeagueResult.data.length > 0) {
        for (let j = 0; j < franchiseLeagueResult.data.length; j++) {
            ...
            let leagueDetail = await LeagueHelper.findLeagueByIdWithDeleteWithPopulate(leagueId)
            if (leagueDetail != null) {
                let gameName = leagueDetail.gameToPlay.gameName
                ...
                let leagueResultObj = {
                    ...
                    total: parseInt((franchiseLeagueResult.data[j].killPoints) + parseInt(franchiseLeagueResult.data[j].placePoints)),
...
exports.scheduleForFourTeams = async (teamArr, maxRoundNumber, leagueId) => {
    let roundNumber = parseInt(maxRoundNumber) + 1
    let checkLeagueScheduleExist = await LeagueScheduleHelper.findLeagueScheduleByLeagueId(leagueId)
    if (checkLeagueScheduleExist.length > 0) {
        let checkLeagueCompleteAndWinnerByPlayoff = await LeagueScheduleHelper.checkAlreadyLeagueScheduleForPlayoff(leagueId, roundNumber)
...
exports.scheduleForTwoTeams = async (maxRoundNumber, leagueId) => {
    ...
    let firstTeam
    let secondTeam
    let matchOneData = await LeagueScheduleHelper.getPlayOffMatchDataWithRoundNumber(leagueId, maxRoundNumber, 1)//1 is row number(match number)
    ...
    let matchTwoData = await LeagueScheduleHelper.getPlayOffMatchDataWithRoundNumber(leagueId, maxRoundNumber, 2)//2 is row number(match number)
...
exports.scheduleForOneTeam = async (maxRoundNumber, previousRoundWinnerTeamsArr, leagueId) => {
    let checkFinalScheduleRequired = await LeagueScheduleHelper.findLeagueScheduleDetailByRoundNumber(maxRoundNumber, leagueId)
    if (checkFinalScheduleRequired.scheduleType != "final") {
        let oldRoundNumber = parseInt(maxRoundNumber) - 1
        let previousMatchDataToGetFirstQualifyTeam = await LeagueScheduleHelper.getPlayOffMatchDataWithRoundNumber(leagueId, oldRoundNumber, 1)
...
exports.userLeagueJoinedStatus = async (userId, leagueId) => {
    ...
    if (checkUserFranchiseTeam != null) {
        ...
        let checkLeagueJoined = await LeagueHelper.findTeamInLeague(leagueId, teamId)
    ...
    return {franchiseId: franchiseId, teamId: teamId, leagueJoined: leagueJoined}
}